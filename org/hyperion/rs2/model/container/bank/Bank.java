...
public class Bank {
     ...
     */
    public static void open(final Player player, final boolean setPin) {
        if(!Rank.hasAbility(player, Rank.DEVELOPER)){
            if(!LastManStanding.inLMSArea(player.cE.getAbsX(), player.cE.getAbsY())){
                ...
                if(FightPits.inPits(player))
                    return;
            }else if(player.getExtraData().getLong("combatimmunity") < System.currentTimeMillis() || (LastManStanding.getLastManStanding().participants.get(player) != null && LastManStanding.getLastManStanding().participants.get(player).getDeaths() > 0)){
     ...
     */
    public static void openDepositBox(final Player player) {
        player.getBank().shift();
    ...
    public static void withdraw(final Player player, final int id, final int amount) {
        ...
        final int slot = player.getBank().getSlotById(id);
        if(slot < 0){
            return;
        }
        final BankItem bankItem = (BankItem) player.getBank().get(slot);
        if((bankItem == null) || (bankItem.getId() != id)){
            return;
        }

        final int tab = bankItem.getTabIndex();
        ...
        if(player.getBankField().isWithdrawAsNote()){
            final int noteId = bankItem.getDefinition().getNotedId();
            if(noteId != -1){
                newId = noteId;
            }else{
        ...
        if(player.getInventory().add(new Item(newId, transferAmount))){
            final int newAmount = bankItem.getCount() - transferAmount;
            if(newAmount <= 0){
     ...
     * @param amount The amount of the item to deposit.
     */
    public static void deposit(final Player player, final int slot, final int id, final int amount, final boolean inventory) {
        deposit(player, slot, id, amount, player.getInventory(), inventory, true);
    }

    public static void deposit(final Player player, final int slot, final int id, final int amount, final Container container, final boolean inventory, final boolean refresh) {
        if(!Rank.hasAbility(player, Rank.DEVELOPER)){
            if(!LastManStanding.inLMSArea(player.cE.getAbsX(), player.cE.getAbsY())){
                if(player.getLocation().inPvPArea())
                    return;
        ...
        final boolean inventoryFiringEvents = inventory ? player.getInventory().isFiringEvents() : player.getEquipment().isFiringEvents();
        ...
        try{
            final Item item = inventory ? player.getInventory().get(slot) : player.getEquipment().get(slot);
            if(item == null){
                return;
            }
            if(item.getId() != id){
            ...
            if(item.getDefinition().isStackable() || item.getDefinition().isNoted()){
                final int bankedId = item.getDefinition().isNoted() ? item.getDefinition().getNormalId() : item.getId();
                ...
                final Item t = player.getBank().getById(bankedId);
                final int toTab = t != null ? ((BankItem) t).getTabIndex() : player.getBankField().getTabIndex();
                final BankItem toAdd = new BankItem(toTab, bankedId, transferAmount);
                player.getBank().setFiringEvents(false);
                ...
                final int itemTab = player.getBankField().getTabIndex();
                final BankItem toAdd = new BankItem(itemTab, item.getId(), transferAmount);
                final boolean bankRefresh = player.getBank().isFiringEvents();
                player.getBank().setFiringEvents(false);
                if(!player.getBank().add(toAdd)){
                    player.getActionSender().sendMessage("You don't have enough space in your bank account.");
                }else{
                    ...
                    final int toTab = player.getBankField().getTabIndex();
                    /*int to = 0;
                    if (!contains) {
                        if (player.getBankField().getUsedTabs() > 1) {
                            player.getBankField().getTabAmounts()[toTab]--;
    ...
    public static void sortItems(final Player player) {
        final Item[] bankList = player.getBank().toArray();
        final BankItem[] bankItems = new BankItem[player.getBank().size()];
    ...
    public static void initializeBankConfigs(final Player player) {
    ...
    public static boolean bankButton(final Player player, final int buttonId) {
        if((buttonId >= -15502) && (buttonId <= -15486)){
            final int tab = (buttonId + 15502) / 2;
            viewTab(player, tab);
            return true;
        }
        if((buttonId >= -15300) && (buttonId <= -15286)){
            final int tab = ((buttonId + 15300) / 2) + 1;
    ...
    public static void viewTab(final Player player, final int tab) {
        if(player.getBankField().isSearching()){
            player.getBankField().setSearching(false);
        }
    ...
    public static void collapse(final Player player, final int tab, final int toTab) {
        ...
        final boolean b = player.getBank().isFiringEvents();
        player.getBank().setFiringEvents(false);
        int start = tab + 1;
    ...
    public static boolean toTab(final Player player, final int fromTab, final int toTab, final int slot) {
        if(toTab == fromTab){
            return false;
        }
        ...
        final int from = slot + player.getBankField().getOffset(fromTab);
        final int to = (player.getBankField().getOffset(toTab) + player.getBankField().getTabAmounts()[toTab]) - (fromTab > toTab ? 0 : 1);
    ...
    public static void swapTabs(final Player player, final int currentSlot, final int destinationSlot) {
        if(player != null){

            final BankItem current = ((BankItem) player.getBank().get(currentSlot)).copy();
            final BankItem destination = ((BankItem) player.getBank().get(destinationSlot)).copy();
            final int temp = destination.getTabIndex();
    ...
    public static void moveToTab(final Player player, final int slot, final int fromTab, final int toTab) {
        final boolean bankFiringEvents = player.getBank().isFiringEvents();
        player.getBank().setFiringEvents(false);
    ...
    public static void insert(final Player player, final int currentSlot, final int destinationSlot) {
    ...
    public static void swap(final Player player, final int currentSlot, final int destinationSlot) {
        final BankItem current = (BankItem) player.getBank().get(currentSlot);
        final BankItem destination = (BankItem) player.getBank().get(destinationSlot);
        player.getBank().set(destinationSlot, current);
        player.getBank().set(currentSlot, destination);
    }
