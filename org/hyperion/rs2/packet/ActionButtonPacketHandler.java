...
import org.hyperion.rs2.model.SkillcapeAnim;
...
public class ActionButtonPacketHandler implements PacketHandler {
    ...
    static {
        CommandHandler.submit(new Command("doaction", Rank.ADMINISTRATOR) {
            ...
            public boolean execute(final Player player, String input) {
                ...
                final int button = Integer.parseInt(input);
    ...
    public static void handle(final Player player, final int button) {
        ...
        switch(button){
            ...
            case 29036:
                ...
                GrandExchangeV2.addItem(player, (Integer) player.getExtraData().get("geitemid"), (Integer) player.getExtraData().get("geitemam"), (Integer) player.getExtraData().get("geitemslot"), (Integer) player.getExtraData().get("geitemprice"));
                break;
            ...
            case 29040:
                ...
                break;
            case 29025:
                player.getExtraData().put("geitemprice", (Integer) player.getExtraData().get("geitemprice") + 1000);
            ...
            case 29030:
                ...
                break;
            case 29023:
                player.getExtraData().put("geitemam", (Integer) player.getExtraData().get("geitemam") + 1);
            ...
            case 2472:
            case 8222:
                DialogueManager.openDialogue(player, player.getInterfaceState().getNextDialogueId(1));
                break;
            case 2473:
            case 8211:
            case 8223:
                DialogueManager.openDialogue(player, player.getInterfaceState().getNextDialogueId(2));
                break;
            case 8212:
            case 8224:
                DialogueManager.openDialogue(player, player.getInterfaceState().getNextDialogueId(3));
                break;
            case 8225:
                DialogueManager.openDialogue(player, player.getInterfaceState().getNextDialogueId(4));
                break;
            case 30306:
		 ...
		 */
            ...
            case 21016:
                synchronized(player){
                    ...
                    for(final Item item : player.getEquipment().toArray()){
                        index2++;
                        if(item != null){
                            Bank.deposit(player, index2, item.getId(), item.getCount(), player.getEquipment(), false, true);
                        }
                    }
                }
                break;
            ...
            case 7612:
                if(player.duelAttackable > 0 && player.duelRule[10]){
                    player.getActionSender().sendMessage("You cannot use special in this duel.");
                    return;
                }
                if(!player.specOn)
                    player.specOn = true;
            ...
            case 27654:
                ...
                final LinkedList<Item> keepItems = new LinkedList<Item>();
                for(int j = 0; j < 4; j++){
                ...
                for(int j = 0; j < player.getInventory().capacity(); j++){
                    final Item item = player.getInventory().get(j);
                ...
                for(int j = 0; j < player.getEquipment().capacity(); j++){
                    final Item item = player.getEquipment().get(j);
                    if(item != null && !keepItems.contains(item)){
                        player.getActionSender().sendUpdateItem(10600, indexcounter++, item);
                    }
                }
                final Item item = new Item(-1, 1);
            ...
            case 17022:
                ...
                final Location newlocation = player.getLocation().getCloseLocation();
    ...
    public void handle(final Player player, final Packet packet) {
        ...
        final ButtonAction action = ActionsManager.getManager().getButtonAction(button);
        if(action != null)
