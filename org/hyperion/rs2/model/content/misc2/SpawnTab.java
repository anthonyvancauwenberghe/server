...
public class SpawnTab {
    ...
    static {
        int index = 0;
        for(final Supplies supplies : Supplies.values()){
            ActionsManager.getManager().submit(START_INDEX + index + 1, new ButtonAction() {
                ...
                public void handle(final Player player, final int id) {
                    if(Rank.hasAbility(player, Rank.DEVELOPER) || ItemSpawning.canSpawn(player, true))
                        for(final Item item : supplies.items){
        ...
        index += 8;
        for(final Sets sets : Sets.values()){
            ActionsManager.getManager().submit(START_INDEX + index + 1, new ButtonAction() {
                @Override
                public void handle(final Player player, final int id) {
                    if(Rank.hasAbility(player, Rank.DEVELOPER) || ItemSpawning.canSpawn(player, true))
                        for(final Item item : sets.items){
                            player.getInventory().add(item);
                        }
                }
            });
    ...
    public SpawnTab(final Player player) {
        this.player = player;
    }
    ...
    public enum Sets {
        MELEE_SET(Item.create(4151, 1), Item.create(7462, 1), Item.create(11732, 1), Item.create(6585, 1), Item.create(4728, 1), Item.create(4730, 1), Item.create(6524, 1), Item.create(10828, 1)),
        MAGE_SET(Item.create(4708, 1), Item.create(4712, 1), Item.create(4714, 1), Item.create(4675, 1), Item.create(6889, 1), Item.create(6920, 1), Item.create(1052, 1), Item.create(1708, 1)),
        RANGE_SET(Item.create(2581, 1), Item.create(2577, 1), Item.create(15126, 1), Item.create(4736, 1), Item.create(4738, 1), Item.create(10499, 1), Item.create(9244, 100), Item.create(9185, 1), Item.create(7462, 1)),
        HYBRID_SET(Item.create(4675, 1), Item.create(4151, 1), Item.create(6585, 1), Item.create(7462, 1), Item.create(6920, 1), Item.create(4712, 1), Item.create(4714, 1), Item.create(4728, 1), Item.create(4730, 1), Item.create(1052, 1), Item.create(10828, 1), Item.create(1215, 1));

        private final Item[] items;

        Sets(final Item... items) {
            this.items = items;
        }
