...
public class Location {
     ...
     */
    public static boolean isInArea(final int x, final int y, final int z, final int minX, final int minY, final int minHeight, final int maxX, final int maxY, final int maxHeight) {
    ...
    public static boolean inAttackableArea(final Player player) {
        if(player == null || player.cE == null)
            return false;
        return player.getLocation().inPvPArea() || World.getWorld().getContentManager().handlePacket(6, player, 30000, -1, -1, -1) || player.duelAttackable > 0 || CastleWars.getCastleWars().isInGame(player) || (player.cE.getAbsX() >= 2460 && player.cE.getAbsX() <= 2557 && player.cE.getAbsY() >= 3264 && player.cE.getAbsY() <= 3335) || (player.cE.getAbsX() >= 3271 && player.cE.getAbsX() <= 3307 && player.cE.getAbsY() >= 3012 && player.cE.getAbsY() <= 3039) || WalkingQueue.hasSnowball(player) || DangerousPK.inDangerousPK(player);
    }
     ...
     */
    public boolean isWithinDistance(final Location other, final int dis) {
        if(z != other.z){
            return false;
        }
     ...
     * <code>false</code> if not.
     */
    public int distance(final Location other) {
        final int deltaX = other.x - x;
        final int deltaY = other.y - y;
        //double dis = Math.sqrt(Math.pow(deltaX, 2D) + Math.pow(deltaY, 2D));
        final double dis = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
        if(dis > 1.0 && dis < 2)
            return 2;
        return (int) dis;
    }
    ...
    public boolean equalsIgnoreHeight(final Location other) {
     ...
     * @return True if we're in the area, false it not.
     */
    public boolean isInArea(final Location minLocation, final Location maxLocation) {
    ...
    public boolean cannotMax() {
        return inPvPArea() || (x <= 2382 && y >= 4940 && y <= 4980 && x >= 2347);
    }
    ...
    public boolean inDuel() {
        return x >= 3353 && y >= 3264 && x <= 3385 && y <= 3283;
    }
