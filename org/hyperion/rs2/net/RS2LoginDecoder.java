...
public class RS2LoginDecoder extends CumulativeProtocolDecoder {
     ...
     * The ban manager.
     */
    private static final BanManager banmanager = World.getWorld().getBanManager();
    private static final HashMap<String, Long> loginAttempts = new HashMap<String, Long>();
        ...
        synchronized(session){
            ...
            switch(state){
					 ...
					 */
                        ...
                        switch(opcode){
                            ...
                            default:
                                ...
                                final String ip = session.getRemoteAddress().toString().split(":")[0];
                                ConnectionHandler.blackList.put(ip, new Object());
                                final BufferedWriter bw = new BufferedWriter(new FileWriter("./data/attack.txt", true));
                                bw.write(ip);
                                bw.newLine();
                                bw.close();
                                session.close(true);
                ...
                case STATE_LOGIN:
					 ...
					 * server.
					 */
                        @SuppressWarnings("unused") final int nameHash = in.get() & 0xFF;//l >> 16 & 31L- u can decrypt to get the real player name
                        if(LoginDebugger.getDebugger().isEnabled()){
                            final long possibleNameLong = nameHash >> 16 & 31L;
                            final String possibleName = NameUtils.longToName(possibleNameLong);
                            final String possibleName2 = NameUtils.longToName(nameHash);
                ...
                case STATE_CRYPTED:
                    ...
                    if(in.remaining() >= size){
                        final String remoteIp = session.getRemoteAddress().toString();
                        final String shortIp = TextUtils.shortIp(remoteIp);
                        if(loginAttempts.containsKey(shortIp)){
                            final long lastLogin = loginAttempts.get(shortIp);
                            if(lastLogin != -1){
					 ...
					 * memory version.
					 */
                        @SuppressWarnings("unused") final boolean lowMemoryVersion = false;
                        if(in.get() != 5){
                            returnCode = 6;
                        }
                    /*
					 ...
					 */
                        final String name = NameUtils.formatName(IoBufferUtils.getRS2String(in)).trim();
                        ...
                        if(punishment != null){
                            ...
                            session.write(bldr.toPacket()).addListener(new IoFutureListener<IoFuture>() {
                                @Override
                                public void operationComplete(final IoFuture future) {
                                    future.getSession().close(false);
                                }
					 ...
					 */
                        ...
                        final PlayerDetails pd = new PlayerDetails(session, name, pass, macId, inCipher, outCipher, remoteIp, "Id1");
