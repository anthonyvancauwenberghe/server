...
public class YellCommand extends Command {
    ...
    private int getYellDelay(final Player player) {
    ...
    public boolean execute(final Player player, String input) {
        ...
        final long yellMilliseconds = (long) (System.currentTimeMillis() - player.getPermExtraData().getLong("yelltimur"));
        final long yellDelay = getYellDelay(player);
        ...
        if(!Rank.isStaffMember(player) && !Server.NAME.equalsIgnoreCase("ArteroBeta")){
            World.getWorld().submit(new Event(yellDelay) {
                public void execute() {
    ...
    public String getTag(final Player player) {
