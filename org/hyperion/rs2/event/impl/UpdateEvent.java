...
public class UpdateEvent extends Event {
    ...
    @Override
    public void execute() {
        final int npcscount = World.getWorld().getNPCs().size();
        final int playercount = World.getWorld().getPlayers().size();
        for(final NPC npc : World.getWorld().npcsWaitingList){
        ...
        final List<Task> tickTasks = new ArrayList<Task>(npcscount + playercount);
        final List<Task> updateTasks = new ArrayList<Task>(playercount);
        final List<Task> resetTasks = new ArrayList<Task>(npcscount + playercount);
        ...
        final Task tickTask = new ConsecutiveTask(tickTasks);
        final Task updateTask = new ParallelTask(updateTasks);
        final Task resetTask = new ParallelTask(resetTasks);
