...
public class NPCManager {
        ...
        CommandHandler.submit(new Command("npcids", Rank.MODERATOR) {
            ...
            public boolean execute(final Player player, final String input) throws Exception {
                for(final NPC npc : World.getWorld().getNPCs()){
                    if(npc == null)
                        continue;
                    final int distance = player.getLocation().distance(npc.getLocation());
                    if(distance < 5){
        ...
        try{
            ...
            while(true){
                ...
                if(spot > -1){
					 ...
					 */
                    final NPC npc = addNPC(Integer.valueOf(valuesArray[1]), Integer.valueOf(valuesArray[2]), Integer.valueOf(valuesArray[3]), id, 60);
                    ...
                    if(id == 495 && banker == null)
                        banker = npc;
                    positionMap.put((Integer.valueOf(valuesArray[1]) * 16 + Integer.valueOf(valuesArray[2]) * 4), npc);
                }
            }
    ...
    public static int getCharms(final int npcId, String name) {
        if(npcId >= 4278 && npcId <= 4284)
            return 0;
        final NPCDefinition nD = NPCDefinition.forId(npcId);
        if(nD.combat() < 10)
            return 0;
        name = name.toLowerCase();
        final int random = Misc.random(20);
    ...
    public static int getTalismine(final NPCDefinition nD) {
        if(nD.combat() < 10)
            return 0;
        final int random = Misc.random(20);
        if(random != 1)
    ...
    private static int getHighTali() {
        ...
        final int[] array = {1452, 1462,};
        return array[Misc.random(array.length - 1)];
    }

    private static int getLowTali() {
        final int[] array = {1438, 1440, 1442, 1444, 1446, 1448,};
        return array[Misc.random(array.length - 1)];
    }

    private static int getMedTali() {
        final int[] array = {1454,};
        return array[Misc.random(array.length - 1)];
    }

    private static int getVHighTali() {
        final int[] array = {1450, 1456, 1458, 1460, 5516,};
        return array[Misc.random(array.length - 1)];
    }

    public static final int getAgreDis(final int npcId) {
        switch(npcId){
            case 7135:
            case 7134:
                return 15;
    ...
    public void restoreArea(final Location location) throws IOException {
        // NPC npc =
        ...
        final String name = "./data/spawnsold.cfg";
        BufferedReader file = null;
        try{
            file = new BufferedReader(new FileReader(name));
            while(true){
                ...
                if(spot > -1){
                    ...
                    final String[] valuesArray = values.split("\t");
                    final int id = Integer.valueOf(valuesArray[0]);
                    final Location l = Location.create(Integer.valueOf(valuesArray[1]), Integer.valueOf(valuesArray[2]), Integer.valueOf(valuesArray[3]));
                    if(location.distance(l) <= 64 && positionMap.get((Integer.valueOf(valuesArray[1]) * 16 + Integer.valueOf(valuesArray[2]) * 4)) == null){
                        final NPC npc = World.getWorld().getNPCManager().addNPC(Integer.valueOf(valuesArray[1]), Integer.valueOf(valuesArray[2]), Integer.valueOf(valuesArray[3]), id, 60);
                        TextUtils.writeToFile("./data/spawns.cfg", "spawn = " + id + "	" + l.getX() + "	" + l.getY() + "	" + l.getZ() + "	" + (l.getX() - 2) + "	" + (l.getY() - 2) + "	" + (l.getX() + 2) + "	" + (l.getY() + 2) + "	1	" + NPCDefinition.forId(id).name());
                        positionMap.put((Integer.valueOf(valuesArray[1]) * 16 + Integer.valueOf(valuesArray[2]) * 4), npc);
                    }
                    /*
    ...
    private int getBones(final int id, String name) {
        name = name.toLowerCase();
        if(id >= 4278 && id <= 4284)
            return 0;
    ...
    public NPC addNPC(final Location loc, final int npcId, final int respawnTime) {
        final NPCDefinition nD = NPCDefinition.forId(npcId);
        final NPC n = new NPC(nD, respawnTime, loc);
    ...
    public NPC addNPC(final int x, final int y, final int z, final int npcId, final int respawnTime) {
    ...
    public void startUp() {
        try{
            int counter = 0;
            final Class[] classes = ClassUtils.getClasses("org.hyperion.rs2.model.combat.attack");
            for(final Class cls : classes){
                if(Attack.class.isAssignableFrom(cls) && cls != Attack.class){
                    try{
                        final Attack attack = (Attack) cls.newInstance();
                        final int[] ids = attack.npcIds();
                        if(ids[0] == -1){
    ...
    public boolean trodOnNpc(final NPC npc, final int i, final int j) {
        final int k = 1 + npc.getDefinition().sizeX();
        final int l = 1 + npc.getDefinition().sizeY();
        final int[][] ai = new int[16][2];
        ...
        for(final NPC npc1 : World.getWorld().getRegionManager().getLocalNpcs(npc)){
            if(npc != npc1 && !npc1.isHidden()){
                int k1 = npc1.getLocation().getX();
                int l1 = npc1.getLocation().getY();
                final WalkingQueue.Point point = npc1.getWalkingQueue().getPublicPoint();
                if(point != null){
                    k1 = point.getX();
                    l1 = point.getY();
                }
    ...
    public Attack getAttack(final NPC n) {
