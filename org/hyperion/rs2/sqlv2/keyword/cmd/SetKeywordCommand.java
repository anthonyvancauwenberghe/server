package org.hyperion.rs2.sqlv2.keyword.cmd;
...
import org.hyperion.rs2.sqlv2.keyword.Keyword;
...
public class SetKeywordCommand extends Command {
    ...
    public boolean execute(final Player player, final String input) throws Exception {
        ...
        if(existing != null){
            ...
            if(!DbHub.initialized() || !DbHub.donationsDb().enabled() || !DbHub.donationsDb().keywords().delete(existing)){
        ...
        if(!DbHub.initialized() || !DbHub.donationsDb().enabled() || !DbHub.donationsDb().keywords().insert(keyword))
