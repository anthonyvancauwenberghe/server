...
public class NightGazer implements Attack {
    ...
    private void handleShardSpecial(final NPC npc) {
        npc.forceMessage("TRY AND DODGE THIS!");
        npc.cE.doAnim(13428);
        for(final Player player : World.getWorld().getRegionManager().getLocalPlayers(npc)){
            final int skill = Combat.random(6);
            if(skill == 3 || skill == 5){
                return;
            }else{
            ...
            final int type = Combat.random(1) + 1;
