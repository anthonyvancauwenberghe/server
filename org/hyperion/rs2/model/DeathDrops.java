...
public class DeathDrops {
    ...
    private static boolean dontDrop(final Player player) {
        return dontDropRank(player.getPlayerRank()) || player.getLocation().inFunPk();
    }

    private static boolean dontDropRank(final long l) {
     ...
     */
    public static void dropsAtDeath(final Player player, final Player killer) {
         ...
         * Adds EP Drop.
         */
        final Item epItems = EPDrops.getEPItem(killer.EP);
        if(epItems != null){
            killer.removeEP();
            World.getWorld().getGlobalItemManager().newDropItem(killer, new GlobalItem(killer, player.getLocation(), epItems));
        }

        final List<Item> droppingItems = dropItems(player, DangerousPK.inDangerousPK(player));
        /**
         * Drops the items for the killer
         */
        for(final Item item : droppingItems){
            ...
            if(killer.getGameMode() <= player.getGameMode())
                ...
                final int price = (int) (NewGameMode.getUnitPrice(item) * NewGameMode.SELL_REDUCTION);
                if(price > 1)
                    World.getWorld().getGlobalItemManager().newDropItem(killer, new GlobalItem(killer, player.getLocation(), Item.create(995, price)));
            }
        }
    ...
    public static final List<Item> dropItems(final Player player, final boolean loseAll) {
        /**
        ...
        final List<Item> keepItems = itemsKeptOnDeath(player, false, false);
     ...
     */
    public static List<Item> processContainer(final Container container, final List<Item> originalDrops, final boolean inv, final Player player) {
    ...
    private static boolean toDrop(final Item item, final int gameMode) {
        if(item == null)
            return false;
        if(ItemsTradeable.isTradeable(item.getId())){
    ...
    public static List<Item> itemsKeptOnDeath(final Player player, final boolean delete, final boolean interfaceUse) {
        final boolean[] invSlot = player.invSlot.clone();
        final boolean[] equipSlot = player.equipSlot.clone();
        final List<Item> keepItems = new LinkedList<Item>();
        final int keeping = player.getLocation().inFunPk() ? player.getInventory().size() + player.getEquipment().size() : ((player.isSkulled() ? 0 : 3) + (player.getPrayers().isProtectingItem() ? 1 : 0));
        for(int i = 0; i < keeping; i++){
            final Item item = keepItem(player, i, delete);
            if(item != null)
                keepItems.add(item);
        }
        if(interfaceUse){
            System.arraycopy(invSlot, 0, player.invSlot, 0, player.invSlot.length);
            System.arraycopy(equipSlot, 0, player.equipSlot, 0, player.equipSlot.length);
        }
        return keepItems;
    }

    @Deprecated
    public static void dropAllItems(final Player player, final Player killer) {
        ...
        player.resetDeathItemsVariables();
        final List<Item> keepItems = itemsKeptOnDeath(player, true, false);
        /**
         * Use one list to avoid repeating code.
         */
        final LinkedList<Item> droppingItems = new LinkedList<Item>();
        Collections.addAll(droppingItems, player.getEquipment().toArray());
        Collections.addAll(droppingItems, player.getInventory().toArray());
        /**
        ...
        if(killer != null && !killer.loggedOut){
            ...
            final Item EPItem = EPDrops.getEPItem(killer.EP);
            if(EPItem != null){
                killer.removeEP();
                if(Rank.hasAbility(killer, Rank.OWNER))
        ...
        final LinkedList<Item> delayedDrops = new LinkedList<Item>();
        /**
         * Clean Equipment.
         */
        for(final Item dropItem : droppingItems){
            if(dropItem == null)
                continue;
        ...
        for(final Item delayedItem : delayedDrops){
            World.getWorld().getGlobalItemManager().newDropItem(killer, new GlobalItem(killer, player.getLocation(), delayedItem));
        }
        for(final Item keepItem : keepItems){
            if(keepItem != null)
                player.getInventory().add(keepItem);
        }
    }

    public static Item keepItem2(final Player player, final int keepItem, final boolean deleteItem) {
        final int capacity = player.getInventory().capacity() + player.getEquipment().capacity();
        int itemId = -1, itemSlot = 0, itemCount = 1, itemContainer = 0;
        for(int i = 0; i < capacity; i++){
            final boolean slotInInv = i < player.getInventory().capacity();
            final int currentSlot = slotInInv ? i : i - player.getInventory().capacity();
            final Item item = (slotInInv ? player.getInventory() : player.getEquipment()).get(currentSlot);
    ...
    public static int calculateAlchValue(final int gameMode, int id) {
        ...
        final int dpVal = DonatorShop.getPrice(id);
    ...
    public static Item keepItem(final Player player, final int keepItem, final boolean deleteItem) {
        int value = 0;
        int item = -1;
        int slotId = 0;
        boolean itemInInventory = false;
        final boolean itemStackZero = true;
        for(int i = 0; i < player.getInventory().capacity(); i++){
            if(player.getInventory().get(i) != null){
                final int dpVal = DonatorShop.getPrice(player.getInventory().get(i).getId());
                final int inventoryItemValue = calculateAlchValue(player.getGameMode(), player.getInventory().get(i).getId());
        ...
        for(int i1 = 0; i1 < player.getEquipment().capacity(); i1++){
            if(player.getEquipment().get(i1) != null){
                final int dpValue = (int) Math.floor(DonatorShop.getPrice(player.getEquipment().get(i1).getId()));
                final int equipmentItemValue = calculateAlchValue(player.getGameMode(), player.getEquipment().get(i1).getId());
