...
public class DonationRequest extends SQLRequest {
     ...
     */
    public DonationRequest(final Player player) {
    ...
    public void process(final SQLConnection sql) {
        try{
            ...
            final String name = player.getName().toLowerCase().replaceAll("_", " ");
            final ResultSet rs = sql.query("SELECT * FROM donator WHERE finished = 0 AND name = '" + name + "'");
            if(rs == null){
            ...
            while(rs.next()){
                final String amountString = rs.getString("amount");
                try{
                    final int toAdd = (int) (Double.parseDouble(amountString) * 100);
            ...
            sql.query("UPDATE donator SET finished = 1 WHERE name = '" + name + "'");
            if(amount >= 200000){
                sql.stopRunning();
