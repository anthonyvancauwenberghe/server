...
public class WildernessBossEvent extends Event {
    ...
    private static final Location[] SPAWN_POINTS = {Location.create(3064, 3774, 0), Location.create(3144, 3778, 0),
            Location.create(3255, 3884, 0), Location.create(3133, 3846, 0), Location.create(3006, 3825, 0)};
     ...
     */
    public WildernessBossEvent(final boolean forceSpawn) {
    ...
    public static void init() {
        ...
        NPCDefinition.getDefinitions()[BOSS_IDS[index]] = NPCDefinition.create(BOSS_IDS[index++], 1025, 425, bonus, 13602, 13601, new int[]{
                13603}, 2, "Bal'lak the Pummeller", -1);
        Arrays.fill(bonus, 370);
        NPCDefinition.getDefinitions()[BOSS_IDS[index]] = NPCDefinition.create(BOSS_IDS[index++], 1050, 420, bonus, 13424, 13420, new int[]{
                13430}, 2, "Night-gazer Khighorahk", -1);
        Arrays.fill(bonus, 380);
        NPCDefinition.getDefinitions()[BOSS_IDS[index]] = NPCDefinition.create(BOSS_IDS[index++], 1100, 430, bonus, 13005, 13000, new int[]{
                13001}, 4, "Bulwark Beast", -1);
        Arrays.fill(bonus, 365);
        NPCDefinition.getDefinitions()[BOSS_IDS[index]] = NPCDefinition.create(BOSS_IDS[index++], 1075, 400, bonus, 13171, 13167, new int[]{
                13170}, 2, "Unholy Cursebearer", -1);
    }

    public static boolean isWildernessBoss(final int npcId) {
        for(final int id : BOSS_IDS)
            if(id == npcId)
                return true;
        return false;
    }
