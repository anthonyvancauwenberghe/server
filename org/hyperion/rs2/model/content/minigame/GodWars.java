...
public class GodWars implements ContentTemplate {
    ...
    private final Map<Integer, Object> bandos = new HashMap<Integer, Object>();
    private final Map<Integer, Object> zammy = new HashMap<Integer, Object>();
    private final Map<Integer, Object> sara = new HashMap<Integer, Object>();
    private final Map<Integer, Object> armdayl = new HashMap<Integer, Object>();

    public static boolean inGodwars(final Player player) {
    ...
    @Override
    public boolean clickObject(final Player player, final int type, final int objId, final int b, final int c, final int d) {
    ...
    public void killBoss(final int bossId, final int x, final int y) {

    }

    public void checkGodWarsInterface(final Player player) {
        if(inGodwars(player)){
    ...
    public int[] getValues(final int type) {
        // TODO Auto-generated method stub
        if(type == 6){
            final int[] j = {26293, 26425,};
            return j;
        }else if(type == 16){
            final int[] j = new int[(bandos.size() + zammy.size() + sara.size() + armdayl.size() + 1)];//monsters that count to kill count
            int i = 0;
            for(final Map.Entry<Integer, Object> entry : bandos.entrySet()){
                j[i++] = entry.getKey();
            }
            for(final Map.Entry<Integer, Object> entry : zammy.entrySet()){
                j[i++] = entry.getKey();
            }
            for(final Map.Entry<Integer, Object> entry : sara.entrySet()){
                j[i++] = entry.getKey();
            }
            for(final Map.Entry<Integer, Object> entry : armdayl.entrySet()){
                j[i++] = entry.getKey();
            }
            return j;
        }
