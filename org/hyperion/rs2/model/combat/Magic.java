...
public class Magic {
    ...
    public static final int[] AUTOCAST_IDS = {/* normal */1830, 1831, 1832, 1833, 1834, 1835, 1836, 1837, 1838, 1839,
            1840, 1841, 1842, 1843, 1844, 1845,/* ancients */13189, 13241, 13147, 6162, 13215, 13267, 13167, 13125,
            13202, 13254, 13158, 13114, 13228, 13280, 13178, 13136,};
    public static final int[] AUTO_MAGIC_IDS = {1152, 1154, 1156, 1158, 1160, 1163, 1166, 1169, 1172, 1175, 1177, 1181,
            1183, 1185, 1188, 1189, 12939, 12987, 12901, 12861, 12963, 13011, 12919, 12881, 12951, 12999, 12911, 12871,
            12975, 13023, 12929, 12891};
    public static final int SPELL_FAIL = 0, SPELL_NEGATIVE = 1, SPELL_SUCCESFUL = 2;
    private static final int EXPMULTIPLIER = 2 * Constants.XPRATE;
    private static final HashMap<Integer, Spell> map = new HashMap<Integer, Spell>();
    ...
    public static int getAutoCastId(final int actionButton) {
    ...
    public static int castSpell(final CombatEntity attacker, final CombatEntity opp, final int spellId) {
        if(attacker.getEntity().isDead() || opp.getEntity().isDead())
            return 0;
        final String message = Combat.canAtk(attacker, opp);
        if(message.length() > 1){
        ...
        if(opp.getEntity() instanceof NPC){
            final String[] FAMILIARS = {"wolpertinger", "steel titan", "yak",
                    "unicorn stallion"};//temp shitfix by fuzen
            for(final String familiarName : FAMILIARS)
        ...
        if(opp.getEntity().isDead())
            return 0;
        if(opp.getOpponent() != attacker && opp.getEntity() instanceof Player && Combat.getWildLevel(attacker.getAbsX(), attacker.getAbsY()) != -1){
         ...
         */
        final int runeIdMissing = hasRunes(attacker, spell);
        if(runeIdMissing != -1){
        ...
        if(spell.getStaffRequired() > 0 && attacker.getPlayer().getEquipment().get(Equipment.SLOT_WEAPON).getId() != spell.getStaffRequired()){
            attacker.getPlayer().getActionSender().sendMessage("You need " + ItemDefinition.forId(spell.getStaffRequired()).getName() + " to cast this spell.");
            attacker.setOpponent(null);
            return 0;
        }
        ...
        if(attacker.getPlayer().getSkills().getLevel(Skills.MAGIC) < spell.getMagicLevel()){
            attacker.getPlayer().getActionSender().sendMessage("You need a magic level of " + spell.getMagicLevel() + " to cast this spell.");
            attacker.setOpponent(null);
            return 0;
        }
        for(final CombatEntity opponent : getMultiPeople(spell, attacker, opp)){
            /**
            ...
            if(attacker.getPlayer().getEquipment().get(Equipment.SLOT_AMULET) != null){
                necklaceId = attacker.getPlayer().getEquipment().get(Equipment.SLOT_AMULET).getId();

            }

            if(attacker.getPlayer().getEquipment().get(Equipment.SLOT_WEAPON) != null){
                weaponId = attacker.getPlayer().getEquipment().get(Equipment.SLOT_WEAPON).getId();
            }
            if(attacker.getEntity() instanceof Player){
                final Player atk = attacker.getPlayer();
                if(atk != null){
                    final int diff = atk.getSkills().getLevel(Skills.MAGIC) - atk.getSkills().getRealLevels()[Skills.MAGIC];
                    if(diff >= 1){
                        //atk.getActionSender().sendMessage("Diff: "+diff );
                        final double multiplication = 1 + ((double) diff * .02);
            ...
            if(opponent.getEntity() instanceof Player){
                ...
                if(opponent.getPlayer().getEquipment().get(Equipment.SLOT_SHIELD) != null){
                    shieldId = opponent.getPlayer().getEquipment().get(Equipment.SLOT_SHIELD).getId();
                }
            }else /** NPC Part */{
                DefBonus = opponent.getNPC().getDefinition().getBonus()[8];
                if(SlayerTask.getLevelById(opponent.getNPC().getDefinition().getId()) > attacker.getPlayer().getSkills().getLevel(Skills.SLAYER)){
                    splash = true;
                }
            }
            ...
            if(spell.isMulti())
                AtkBonus *= 0.9;
            final int deltaBonus = AtkBonus - DefBonus;
            final int toAdd = Misc.random(deltaBonus / 3);
            if(spellId != 12445)
                Damage += toAdd;
            ...
            if(opponent.getEntity() instanceof Player){
                ...
                if(spell.getSpellId() == 12445 && opponent.getPlayer().isTeleBlocked()){
                    attacker.getPlayer().getActionSender().sendMessage("This player is already teleblocked.");
                    return 0;
                }

            }

            final double reduction = Damage <= 0 ? 1.1 : 1.3;
        ...
        /*if(spell.getMaxHit() > 0 && Damage <= 0) {
             ...
             * Add Experience
             */
            final int xpMulti = (attacker.getEntity() instanceof Player && attacker.getPlayer().getLocation().inPvPArea()) ? 2 : (EXPMULTIPLIER * 10);
            ...
            if(freeze > 0 && opponent.canMove() && !splash && opponent.canBeFrozen()){
                ...
                if(opponent.getEntity() instanceof Player)
                    opponent.getPlayer().getActionSender().sendMessage("You have been frozen.");
            ...
            final int offsetY = (attacker.getAbsX() - opponent.getAbsX()) * -1;
            final int offsetX = (attacker.getAbsY() - opponent.getAbsY()) * -1;
            // find our lockon target
            final int hitId = attacker.getSlotId(attacker.getEntity());
            // extra proj values - not to be released
            int timer = 3;
            int speed = 105;
            final int distance = attacker.getEntity().getLocation().distance(opponent.getEntity().getLocation());
            if(distance >= 9){
                timer = 6;
            }else if(distance >= 6){
                timer = 5;
            ...
            min -= (distance - 1) * 8;
            speed -= min;
            final int slope = 12 + distance;

            timer = 1600 + (distance * 200);
            attacker.getPlayer().getActionSender().createGlobalProjectile(attacker.getAbsY(), attacker.getAbsX(), offsetY, offsetX, 50, speed, spell.getMoveGfx(), 43, 35, hitId, slope);
                ...
                public void execute() {
                    ...
                    if(spell.isMulti()){
                        if(Combat.isInMulti(attacker)){
                            finishMagic(attacker, opp2, submitDamage, spell, opp2 == opp ? true : false, submitSplash, critical);
                            hitSomething = true;
                        }
                    }
                    if(!hitSomething)
                        finishMagic(attacker, opp2, submitDamage, spell, true, submitSplash, critical);
                    // delete runes
    ...
    public static void vengeance(final Player player, final CombatEntity victim, final int hit) {
    ...
    public static void recoil(final Player p, final CombatEntity victim, final int hit) {
        if(hit < 10)
            return;
    ...
    public static void preformCharge(final Player player) {
        if((ContentEntity.getItemAmount(player, 554) < 3 || ContentEntity.getItemAmount(player, 556) < 3 || ContentEntity.getItemAmount(player, 565) < 3)){
            ContentEntity.sendMessage(player, "You need more runes to cast the charge.");
            return;
        }
        if(player.hasCharge()){
        ...
        if(player.getSkills().getLevel(6) < 80){
            ContentEntity.sendMessage(player, "You need a magic level of 80 to activate this spell.");
            return;
        }
        if(player.duelRule[4] && player.duelAttackable > 0){
            player.getActionSender().sendMessage("You cannot use magic in this duel.");
            return;
        }
    ...
    public static void clickVengeance(final Player player) {
        if(player.duelRule[4] && player.duelAttackable > 0){
            player.getActionSender().sendMessage("You cannot use magic in this duel.");
            return;
        }
        if(player.getSkills().getLevel(6) < 94){
            ContentEntity.sendMessage(player, "You need 94 magic to cast Vengeance.");
            return;
        }
        if(player.getSkills().getLevelForExp(1) < 40){
            player.getActionSender().sendMessage("You need 40 defence to cast Vengeance.");
            return;
        }
        if((ContentEntity.getItemAmount(player, 9075) < 4 || ContentEntity.getItemAmount(player, 557) < 10 || ContentEntity.getItemAmount(player, 560) < 2) && !Rank.hasAbility(player, Rank.ADMINISTRATOR)){
            ContentEntity.sendMessage(player, "You need more runes to cast Vengeance.");
            return;
        }
        if(player.vengeance){
        ...
        if(nextVeng > 0){
            player.getActionSender().sendMessage("You can only cast Vengeance every" + (vengTimer / 1000) + " seconds (" + nextVeng + "ms remaining)");
            return;
        }
    ...
    public static void clickVenganceOther(final Player caster, final Player player2) {
        if(caster.getSkills().getLevel(6) < 93){
            ContentEntity.sendMessage(caster, "You need 93 magic to cast Vengeance Other.");
            return;
        }
        if(ContentEntity.getItemAmount(caster, 9075) < 3 || ContentEntity.getItemAmount(caster, 557) < 10 || ContentEntity.getItemAmount(caster, 560) < 2){
            ContentEntity.sendMessage(caster, "You need more runes to cast Vengeance Other.");
            return;
        }
        ...
        if(!player2.vengeance){
            if(System.currentTimeMillis() > player2.lastVeng + 30000){
                ...
                caster.getActionSender().sendMessage("You can only cast Vengeance every 30 seconds.");
            }
        }
    }

    public static void finishMagic(final CombatEntity c, final CombatEntity p, final int Damage, final Spell spell, final boolean castedOn, final boolean splash, final boolean critical) {
        if(p.getEntity() instanceof Player){
            if(spell.getSpellId() == 12445 && c.getPlayer().getSpellBook().isRegular() && !splash){
                if(!(p.getPlayer().getPrayers().isEnabled(Prayers.CURSE_DEFLECT_MAGIC) || p.getPlayer().getPrayers().isEnabled(Prayers.PRAYER_PROTECT_FROM_MAGE)))
                    p.getPlayer().setTeleBlock(System.currentTimeMillis() + 300000);
                else
                    p.getPlayer().setTeleBlock(System.currentTimeMillis() + 120000);
                p.getPlayer().getActionSender().sendMessage("You are now teleblocked");
        ...
        if(false/* END_GFX[spell] == 369 && !splash */)
            ...
            final int addCritical = critical ? 5 : 0;
        ...
        if(spell.getPoison() > 0 && !p.isPoisoned() && Combat.random(100) < spell.getPoison() && !splash){
    ...
    public static List<CombatEntity> getMultiPeople(final Spell spell, final CombatEntity caster, final CombatEntity hit) {
        final List<CombatEntity> k = new LinkedList<CombatEntity>();
        if(!Combat.isInMulti(caster) || !spell.isMulti()){
            k.add(hit);
            return k;
        }
        for(final Player p : hit.getEntity().getRegion().getPlayers()){
            if(caster != p.cE && Combat.canAtk(caster, p.cE).length() <= 1 && hit.getEntity().getLocation().isWithinDistance(p.getLocation(), 1)){
                k.add(p.cE);
            }
        }
        for(final NPC n : hit.getEntity().getRegion().getNpcs()){
            if(hit.getEntity().getLocation().isWithinDistance(n.getLocation(), 1) && Combat.isInMulti(n.cE) && !(n.summoned && n.ownerId > 0))
                k.add(n.cE);
        }
        return k;
    }

    public static boolean hasStaff(final CombatEntity c, final int rune) {
        /*
    ...
    public static int hasRunes(final CombatEntity c, final Spell spell) {
        if(c.getEntity() instanceof Player){
            final Player player = (Player) c.getEntity();
            if(Rank.hasAbility(player, Rank.ADMINISTRATOR))
                return -1;
        }
        if(spell.getFirstRune() > 0 && spell.getFirstAmount() > 0)
            if(!hasStaff(c, spell.getFirstRune()) && (c.getPlayer().getInventory().getById(spell.getFirstRune()) == null || c.getPlayer().getInventory().getById(spell.getFirstRune()).getCount() < spell.getFirstAmount()) && (c.getPlayer().getRunePouch().getById(spell.getFirstRune()) == null || c.getPlayer().getRunePouch().getById(spell.getFirstRune()).getCount() < spell.getFirstAmount()))
                return spell.getFirstRune();
        if(spell.getSecondRune() > 0 && spell.getSecondAmount() > 0)
            if(!hasStaff(c, spell.getSecondRune()) && (c.getPlayer().getInventory().getById(spell.getSecondRune()) == null || c.getPlayer().getInventory().getById(spell.getSecondRune()).getCount() < spell.getSecondAmount()) && (c.getPlayer().getRunePouch().getById(spell.getSecondRune()) == null || c.getPlayer().getRunePouch().getById(spell.getSecondRune()).getCount() < spell.getSecondAmount()))
                return spell.getSecondRune();
        if(spell.getThirdRune() > 0 && spell.getThirdAmount() > 0)
            if(!hasStaff(c, spell.getThirdRune()) && (c.getPlayer().getInventory().getById(spell.getThirdRune()) == null || c.getPlayer().getInventory().getById(spell.getThirdRune()).getCount() < spell.getThirdAmount()) && (c.getPlayer().getRunePouch().getById(spell.getThirdRune()) == null || c.getPlayer().getRunePouch().getById(spell.getThirdRune()).getCount() < spell.getThirdAmount()))
                return spell.getThirdRune();
        if(spell.getFourthRune() > 0 && spell.getFourthAmount() > 0)
            if(!hasStaff(c, spell.getFourthRune()) && (c.getPlayer().getInventory().getById(spell.getFourthRune()) == null || c.getPlayer().getInventory().getById(spell.getFourthRune()).getCount() < spell.getFourthAmount()) && (c.getPlayer().getRunePouch().getById(spell.getFourthRune()) == null || c.getPlayer().getRunePouch().getById(spell.getFourthRune()).getCount() < spell.getFourthAmount()))
                return spell.getFourthRune();
        return -1;
    }

    public static boolean deleteRunes(final CombatEntity c, final Spell spell) {
        if(!hasStaff(c, spell.getFirstRune()))
            if(c.getPlayer().getRunePouch().getById(spell.getFirstRune()) == null || c.getPlayer().getRunePouch().getById(spell.getFirstRune()).getCount() < spell.getFirstAmount()){
                c.getPlayer().getInventory().remove(-1, new Item(spell.getFirstRune(), spell.getFirstAmount()));
            }else{
                c.getPlayer().getRunePouch().remove(-1, new Item(spell.getFirstRune(), spell.getFirstAmount()));
            }
        if(!hasStaff(c, spell.getSecondRune()))
            if(c.getPlayer().getRunePouch().getById(spell.getSecondRune()) == null || c.getPlayer().getRunePouch().getById(spell.getSecondRune()).getCount() < spell.getSecondAmount()){
                c.getPlayer().getInventory().remove(-1, new Item(spell.getSecondRune(), spell.getSecondAmount()));
            }else{
                c.getPlayer().getRunePouch().remove(-1, new Item(spell.getSecondRune(), spell.getSecondAmount()));
            }
        if(!hasStaff(c, spell.getThirdRune()))
            if(c.getPlayer().getRunePouch().getById(spell.getThirdRune()) == null || c.getPlayer().getRunePouch().getById(spell.getThirdRune()).getCount() < spell.getThirdAmount()){
                c.getPlayer().getInventory().remove(-1, new Item(spell.getThirdRune(), spell.getThirdAmount()));
            }else{
                c.getPlayer().getRunePouch().remove(-1, new Item(spell.getThirdRune(), spell.getThirdAmount()));
            }
        if(!hasStaff(c, spell.getFourthRune()))
            if(c.getPlayer().getRunePouch().getById(spell.getFourthRune()) == null || c.getPlayer().getRunePouch().getById(spell.getFourthRune()).getCount() < spell.getFourthAmount()){
                c.getPlayer().getInventory().remove(-1, new Item(spell.getFourthRune(), spell.getFourthAmount()));
            }else{
                c.getPlayer().getRunePouch().remove(-1, new Item(spell.getFourthRune(), spell.getFourthAmount()));
            }
        return true;
    }

    /* Misc magic */
    public static void alch(final Player player, final int item, final int slot, final int spell) {
        ...
        if(spell == 1162 || spell == 1178){
            ...
            if(ContentEntity.getItemAmount(player, 561) < 1){
                ContentEntity.sendMessage(player, "You do not have enough nature runes to cast this spell");
                return;
            }
            if(hasStaff(player.cE, 554) || ContentEntity.getItemAmount(player, 554) >= ((spell == 1162) ? 3 : 5)){
                ContentEntity.deleteItemA(player, 554, ((spell == 1162) ? 3 : 5));
                ContentEntity.deleteItemA(player, 561, 1);
                ContentEntity.deleteItem(player, item, slot, 1);
                ContentEntity.startAnimation(player, ((spell == 1162) ? 712 : 713));
                ContentEntity.addItem(player, 995, ((spell == 1162) ? ItemDefinition.forId(item).getLowAlcValue() : ItemDefinition.forId(item).getHighAlcValue()));
                ContentEntity.playerGfx(player, ((spell == 1162) ? 112 : 113), 0);
                ContentEntity.addSkillXP(player, ((spell == 1162) ? 350 : 800), 6);
                player.getActionSender().setViewingSidebar(6);
            }else{
                ContentEntity.sendMessage(player, "You do not have enough fire runes to cast this spell");
            }
        }
    }

    public static void teleport(final Player player, final String location) {
        int x = 3222;
        int y = 3222;
        int z = 0;
    ...
    public static void openTeleMenu(final Player player, final int menu) {
    ...
    public static void clickNewTeleInterface(final Player player, int button) {
        ...
        if(player.getTimeSinceLastTeleport() < 1600)
            return;
        button = button - 3001;
        final int menu = (Integer) player.getExtraData().get("teleMenu");
        if(menu == 0){
            if(button == 0)
                teleport(player, "varrock");
            if(button == 1)
    ...
    public static void homeTeleport(final Player player) {
        ...
        if(!player.getDungeoneering().inDungeon() && (World.getWorld().getContentManager().handlePacket(6, player, 30000, -1, -1, -1) || World.getWorld().getContentManager().handlePacket(6, player, 30001, -1, -1, -1))){
            player.getActionSender().sendMessage("You cannot teleport from fight pits.");
            return;
        }
        ...
        if((player.getLocation().getX() >= 2814 && player.getLocation().getX() <= 2942 && player.getLocation().getY() >= 5250 && player.getLocation().getY() <= 5373) && (x < 2814 || x > 2942 || y < 5250 || y > 5373)){
        ...
        World.getWorld().submit(new Event(600) {
            ...
            public void execute() {
                ...
                if(player.cE.getOpponent() != null){
                    player.getActionSender().sendMessage("You cannot teleport while fighting!");
                    this.stop();
                ...
                ContentEntity.startAnimation(player, Misc.HomeTeleportAnimations[index]);
    ...
    public static void teleport(final Player player, final Location loc, final boolean force) {
        teleport(player, loc.getX(), loc.getY(), loc.getZ(), force, true);
    }

    public static void teleport(final Player player, final Location loc, final boolean force, final boolean random) {
        teleport(player, loc.getX(), loc.getY(), loc.getZ(), force, random);
    }

    public static void teleport(final Player player, final int x, final int y, final int z, final boolean force) {
        teleport(player, x, y, z, force, true);
    }

    public static void teleport(final Player player, int x, int y, final int z, final boolean force, final boolean random) {
        if(!force){
            if(LastManStanding.inLMSArea(player.cE.getAbsX(), player.cE.getAbsY())){
                player.getActionSender().sendMessage("You cannot teleport in this minigame.");
                return;
            ...
            if(World.getWorld().getContentManager().handlePacket(6, player, 30000, -1, -1, -1) || World.getWorld().getContentManager().handlePacket(6, player, 30001, -1, -1, -1)){
                player.getActionSender().sendMessage("You cannot teleport here.");
                return;
            }
            if(player.getTimeSinceLastTeleport() < 1600)
                return;
            ...
            if(player.cE.getOpponent() != null && player.wildernessLevel > 0){
                player.getActionSender().sendMessage("@blu@You have lost EP because you have teleported during combat.");
                player.removeEP();
            }
        }
    ...
    public static void swapSpellbook(final Player p) {
        if(p.getSkills().getLevel(6) < 96){
            ContentEntity.sendMessage(p, "You need 96 magic to cast Spellbook Swap.");
            return;
        }
        if(p.getSkills().getLevelForExp(1) < 40){
            p.getActionSender().sendMessage("You need 40 defence to cast this spell.");
            return;
        }
        if(ContentEntity.getItemAmount(p, 9075) < 3 || ContentEntity.getItemAmount(p, 564) < 2 || ContentEntity.getItemAmount(p, 563) < 1){
            ContentEntity.sendMessage(p, "You need more runes to cast Spellbook Swap.");
            return;
        }
    ...
    public static final boolean in13sArea(final int x, final int y) {
