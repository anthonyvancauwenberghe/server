...
public class AccountValue {
    ...
    public AccountValue(final Player player) {
        this.player = player;
    }

    public static int getContainerValue(final Container container) {
        int counter = 0;
        if(container == null)
            return counter;
     ...
     * @return
     */
    public static int getItemValue(final Item item) {
        if(item == null)
            return 0;
        if(item.getId() == 11694){
            return 0;
    ...
    public long getPkPointValue() {
        final int ge = player.getGrandExchangeTracker().entries.stream(e -> !e.cancelled && !e.progress.completed() && e.type == Entry.Type.BUYING && e.currency == Entry.Currency.PK_TICKETS).mapToInt(e -> e.unitPrice * e.progress.remainingQuantity()).sum() + player.getGrandExchangeTracker().entries.stream(e -> e.claims.progressSlot.holding(5020) || e.claims.returnSlot.holding(5020)).mapToInt(e -> e.claims.progressSlot.holding(5020) ? e.claims.progressSlot.itemQuantity() : e.claims.returnSlot.itemQuantity()).sum();
        return ge + player.getInventory().getCount(5020) + player.getBank().getCount(5020) + (player.getPoints().getPkPoints() / 10);
    }
    ...
    public int getTotalValue() {
        ...
        counter += player.getGrandExchangeTracker().entries.stream(e -> !e.cancelled && !e.progress.completed() && e.type == Entry.Type.SELLING).mapToInt(e -> getItemValue(Item.create(e.itemId, e.progress.remainingQuantity()))).sum();
        counter += player.getGrandExchangeTracker().entries.stream(e -> !e.claims.empty()).mapToInt(e -> {
            int total = 0;
