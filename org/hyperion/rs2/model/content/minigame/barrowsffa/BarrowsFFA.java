...
public class BarrowsFFA extends SpecialArea implements ContentTemplate {
    ...
    public static String toMinutes(final int i) {
        return String.format("%d:%s%d", i / 60, i % 60 >= 10 ? "" : "0", i % 60);
    }

    public static void spawnObject(final List manager) {
        manager.add(new GameObject(PORTAL_ENTER_OBJECT, PORTAL_DEFAULT_LOCATION.transform(0, -1, 0), 10, /*rotation*/ 0, false)); //make a portal 1 space away from people will teleport
    }
    ...
    public boolean dialogueAction(final Player player, final int dialogueId) {
        ...
        switch(dialogueId){
            case DIALOGUE_ID:
                ...
                player.getActionSender().sendDialogue("Select a set", ActionSender.DialogueType.OPTION, -1, Animation.FacialAnimation.DEFAULT, strings);
                for(int i = 0; i < strings.length; i++)
    ...
    public void sendInterfaceString(final Player player, final int i, final String s) {
    ...
    public boolean deathCheck(final Player player, final Player killer) {
        ...
        final boolean one = game.contains(killer);
        final boolean two = game.contains(player);

        System.out.println(one + " | " + two);
        if(one && two){

            final boolean rampage = killer.getBarrowsFFA().kill(player);
            final boolean die = player.getBarrowsFFA().die(killer);
