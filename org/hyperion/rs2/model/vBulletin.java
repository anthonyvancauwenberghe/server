...
public class vBulletin implements Runnable {
	...
	private void connect() {
		...
		try {
			Connection connection2 = DriverManager.getConnection(hostAddress, username, password);
	...
	public int[] checkUser(String name, String password) {
		int[] returnCodes = {8, 0};//return code for client, group id
		if(/*!Server.dedi*/true) {
			//returnCodes[1] = 6;
			return returnCodes;
		}
		...
		try {
			...
			PreparedStatement pStatement = getStatement();
			if(pStatement == null) {
				returnCodes[0] = 8;
				return returnCodes;
			}
