...
public class DungeoneeringManager implements ContentTemplate {
    ...
    public static final boolean cantJoin(final Player player) {
        return !player.getLocation().inDungeonLobby() || ContentEntity.getTotalAmountOfEquipmentItems(player) > 0 || !(ContentEntity.getTotalAmountOfItems(player) == 1 && player.getInventory().contains(15707)) ||  player.cE.summonedNpc != null || (player.getBoB() != null && player.getBoB().freeSlots() != player.getBoB().size());
    }
