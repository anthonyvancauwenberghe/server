...
public class GemCutting extends Crafting {
    ...
    private static Gem getGem(final int i) {
    ...
    public static boolean cutGem(final Player c, final int gem, final int slot) {
        final Gem g = getGem(gem);
        if(g == null){
            return false;
        }
        ...
        World.getWorld().submit(new Event(2200) {
            ...
            public void execute() {
                ...
                final boolean isBolt = g.getName().contains("tip");
                if(isBolt){
    ...
    public enum Gem {
        ...
        Gem(final int gemId, final int resultId, final int exp, final int levelReq, final int emote) {
            this.gemId = gemId;
