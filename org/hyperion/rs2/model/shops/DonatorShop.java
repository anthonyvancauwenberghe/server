...
public class DonatorShop extends Shop {
    ...
    public static final int[][] VEBLEN_GOODS = {{13672, 20000}, {14479, 20000}, {13117, 25000}, //put
            {14494, 20000},};

    private static final HashMap<Integer, Object> donatorItems = new HashMap<Integer, Object>();

    private static boolean first = true;

    public DonatorShop(final int id, final String name, final Container container) {
        super(id, name, container, false);
    }

    public static boolean isVeblenGood(final int itemId) {
        for(final int[] veblen_good : VEBLEN_GOODS){
    ...
    public void sellToShop(final Player player, final Item item) {
        ...
        if(payment > 0){
            ...
            final String query = "INSERT INTO donatorshop(username,item_id,item_count,boughtvalue) " + "VALUES('" + player.getName().toLowerCase() + "'," + item.getId() + "," + item.getCount() + "," + -1 * payment + ")";
    ...
    public void buyFromShop(final Player player, final Item item) {
        ...
        final long lastbuy = player.getExtraData().getLong("lastbuy");
        ...
        if(player.getPoints().getDonatorPoints() >= price){
            ...
            final String query = "INSERT INTO donatorshop(username,item_id,item_count,boughtvalue) " + "VALUES('" + player.getName().toLowerCase() + "'," + item.getId() + "," + item.getCount() + "," + price + ")";
            if(Server.getConfig().getBoolean("logssql"))
                World.getWorld().getLogsConnection().offer(new QueryRequest(query));
            player.getActionSender().sendUpdateItems(3823, player.getInventory().toArray());
            updatePlayers();
            ...
            player.getActionSender().sendMessage("You don't have enough donator points to buy this item.");
        }
    }
    ...
    public void valueBuyItem(final Player player, final Item item) {
        ...
        String message = "The shop will sell a '@dre@" + item.getDefinition().getProperName() + "@bla@' for " + price + " donator points.";
        if(price == 1){
    ...
    public void valueSellItem(final Player player, final Item item) {
        ...
        price *= RESELL_RATE;
        String message = "The shop will buy a '@dre@" + item.getDefinition().getProperName() + "@bla@' for " + price + " donator points.";
        if(price == 1){
            message = message.replace("points", "point");
        }
        player.getActionSender().sendMessage(message);
    ...
    public void process() {
        for(final Item item : getStaticItems()){
            ...
            if(getContainer().contains(item.getId())){
                ...
                if(delta > 0){
                    final int addCount = Math.max(delta / 10, 1);
