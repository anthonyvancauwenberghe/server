...
public class ItemSpawning {
     ...
     * Donator Items.
     */
    private static final String[] DONATOR_NAMES = {"party", "chaotic", "h'ween", "santa", "primal", "3rd", "light",
            "arcane"};

    /**
     * Items that should be exchanged for Pk points.
     */
    private static final String[] PKPOINTS_NAMES = {"void", "defender", "vesta", "statius", "morrigan", "zuriel",
            "spirit",};
     ...
     */
    private static final String[] FORBIDDEN_NAMES = {"zanik", "crate", "charm", "more", "null", "armadyl godsword"};
     ...
     * @param amount
     */
    public static void spawnItem(final Player player, final int id, final int amount) {
        if(!ItemSpawning.canSpawn(player))
            return;
        final String message = allowedMessage(id);
        if(message.length() > 0){
    ...
    public static boolean buy(final int id, final int amount, final Player player) {
        ...
        final int price = (int) full_price;
    ...
    public static void spawnItem(final int id, int amount, final Player player) {
    ...
    public static boolean canSpawn(final int id) {
    ...
    public static boolean canSpawn(final Player player, final boolean msg) {
        ...
        if(player.getLocation().inPvPArea()){
            if(msg)
                player.getActionSender().sendMessage("You cannot do that in a PvP area.");
            return false;
        ...
        if(player.getLocation().inDungeonLobby())
            return false;
        if(World.getWorld().getContentManager().handlePacket(ClickType.OBJECT_CLICK1, player, ClickId.CAN_TELEPORT))
            return false;
     ...
     */
    public static String allowedMessage(final int id) {
        switch(id){
            case Dicing.DICE_ID:
                return "";
        }
        ...
        final String itemName = ItemDefinition.forId(id).getName().toLowerCase();
        for(final String forbiddenName : DONATOR_NAMES){
        ...
        for(final String forbiddenName : PKPOINTS_NAMES){
            if(itemName.contains(forbiddenName))
                return "This item can only be purchased in the " + Server.NAME + " points shop.";
        }
        /**
        ...
        for(final String forbiddenName : UNSPAWNABLE_NAMES){
            if(itemName.contains(forbiddenName))
