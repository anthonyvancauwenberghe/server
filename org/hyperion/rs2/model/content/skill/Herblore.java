...
public class Herblore implements ContentTemplate {
    ...
    public int getIndex(final int herb) {
        for(final Herb c : herbs){
            if(c.getHerbId() == herb)
    ...
    public int getPotionIndex(final int herb) {
        for(final UnfinishedPotion u : unfinishedPotions){
            if(u.getHerb() == herb)
    ...
    public void identifyHerb(final Player c, final int herb, final int slot) {
        final Herb h = herbs.get(getIndex(herb));
    ...
    public void mixPotion(final Player c, int herb, int itemId, int slot, int slot2, final boolean first) {
    ...
    public int getEndPotionIndex(final int potion) {
        for(final Potion pot : potions){
    ...
    public Potion getEndPotionIndex2(final int potion) {
        for(final Potion pot : potions){
            if(pot.getPotionId() == potion)
                return pot;
        }
    ...
    public boolean checkPotion(final Player c, final int potion, final int item) {
        for(final Potion pot : potions){
            if(pot.getPotionId() == potion){
                final int[] items = pot.getSecondItems();
                for(int i = 0; i < items.length; i++){
                    if(items[i] == item)
                        return true;
                }
    ...
    public int getPlace(final int potion, final int item) {
        final int index = getEndPotionIndex(potion);
        if(index == -1)
            return -1;
        final Potion p = potions.get(index);

        final int[] items = p.getSecondItems();
    ...
    public boolean clickObject(final Player client, final int type, int id, final int slot, int itemId2, final int itemSlot2) {
        if(type == 0){
            ...
            final int id1 = client.getSkillingData().getFirstId();
            final int id2 = client.getSkillingData().getSecondId();
            final int slot1 = client.getSkillingData().getFirstSlot();
            final int slot2 = client.getSkillingData().getSecondSlot();
                ...
                try{
                    if(!finishedPotion){
                        if(i >= unfinishedPotions.size())
                            return false;
                        final UnfinishedPotion u = unfinishedPotions.get(i);
                        i = u.getPotion();
                    }else{
    ...
    public int[] getValues(final int type) {
        if(type == 0){
            ...
            final int[] j = new int[herbs.size()];
            int i2 = 0;
            for(final Herb i : herbs){
                j[i2] = i.getHerbId();
                i2++;
            }
            return j;
            ...
            for(final UnfinishedPotion p : unfinishedPotions){
                size += 2;
            }
