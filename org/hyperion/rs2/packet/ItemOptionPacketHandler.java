...
public class ItemOptionPacketHandler implements PacketHandler {
    ...
    private void handleItemOnItem(final Player player, final Packet packet) {
        final int usedWithSlot = packet.getShort();
        final int itemUsedSlot = packet.getShortA();//the item highlighted
        ...
        final int onItem = player.getInventory().get(usedWithSlot).getId();
        final int useItem = player.getInventory().get(itemUsedSlot).getId();
    ...
    private void handleItemOnObject(final Player player, final Packet packet) {
        packet.getLEShortA();
        final int objectId = packet.getLEShort();
        final int objectY = packet.getLEShortA();
        final int itemSlot = (packet.getLEShortA() - 128);
    ...
    private void handleItemOnNpc(final Player player, final Packet packet) {
        final int itemId = packet.getShortA();
        final int i = packet.getShortA();
        final int invslot = packet.getLEShort();
        if(invslot >= player.getInventory().capacity()){
            return;
        }
        final Item item = player.getInventory().get(invslot);
    ...
    private void handleItemOnMagic(final Player player, final Packet packet) {
        final int castOnSlot = packet.getShort();
        final int castOnItem = packet.getShortA();
        /*int interfaceId = */
        packet.getShort();
        final int castSpell = packet.getShortA();
    ...
    private void handleEatOption(final Player player, final Packet packet) {
        final int buryA = packet.getLEShortA();
        final int burySlot = (packet.getShortA());
        final int buryItemID = packet.getLEShort();
        switch(buryItemID){
            case 7510:
            case 7509:
        ...
        switch(interfaceId){
            case Equipment.INTERFACE:
                if(slot >= 0 && slot < Equipment.SIZE){
                    ...
                    if((FightPits.teamBlue.contains(player) && id == FightPits.BLUE_CAPE) || (FightPits.teamRed.contains(player) && id == FightPits.RED_CAPE)){
    ...
    private void handleItemOption6(final Player player, final Packet packet) {
        final int interfaceId = packet.getShortA() & 0xFFFF;
    ...
    private void handleItemOption7(final Player player, final Packet packet) {
    ...
    private void handleItemOption2(final Player player, final Packet packet) {
        ...
        final int slot = packet.getLEShort();
