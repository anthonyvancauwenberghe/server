...
public class BuyBankTab implements ContentTemplate {
    ...
    public boolean dialogueAction(final Player player, final int dialogueId) {
        final int offset = dialogueId == 6503 ? 0 : player.getBankField().getTabAmount() - 2;
        final TabPrice price = TabPrice.values()[offset];
        switch(dialogueId){
            case 6500:
                player.getActionSender().sendDialogue("Banker", ActionSender.DialogueType.NPC, 449, Animation.FacialAnimation.HAPPY, "This bank tab is currently unavailable for you", "You can purchase it for " + price.dp + " dp and " + price.pkp + " pk points!");
                player.getInterfaceState().setNextDialogueId(0, 6501);
                return true;
            case 6501:
                player.getActionSender().sendDialogue("Purchase", ActionSender.DialogueType.OPTION, 449, Animation.FacialAnimation.DEFAULT, "Yes! I would like to purchase it", "Nevermind...");
            ...
            case 6502:
                ...
                if(points.getPkPoints() < price.pkp || points.getDonatorPoints() < price.dp){
                    player.getActionSender().sendDialogue("Banker", ActionSender.DialogueType.NPC, 449, Animation.FacialAnimation.ANGER_1, "You don't have enough points to purchase another bank tab!");
                ...
                player.getActionSender().sendDialogue("Banker", ActionSender.DialogueType.NPC, 449, Animation.FacialAnimation.ANGER_1, "Thankyou very much. You can now use the extra bank tab!");
                player.getInterfaceState().setNextDialogueId(0, 6503);
                return true;
            case 6503:
    ...
    private enum TabPrice {
        ...
        TabPrice(final int pkt) {
            this(pkt, 0);
        }

        TabPrice(final int pkp, final int dp) {
            this.dp = dp;
            this.pkp = pkp;
        }
    }
