        ...
        try(final PreparedStatement stmt = sql.prepare("SELECT * FROM achievement_data WHERE playerName = ?")){
            ...
            try(final ResultSet rs = stmt.executeQuery()){
                while(rs.next()){
                    ...
                    final Timestamp startDate = Optional.ofNullable(rs.getTimestamp("startTime")).orElse(null);
                    final Timestamp finishDate = Optional.ofNullable(rs.getTimestamp("finishTime")).orElse(null);
