...
public class MiningV2 implements ContentTemplate {
    ...
    private static final Map<Integer, Pickaxe> pickaxes = new HashMap<Integer, Pickaxe>();
    private final Map<Integer, Rock> rockOres = new HashMap<Integer, Rock>();
    private final Map<Location, Integer> rockLocationStatus = new HashMap<Location, Integer>();

    public static Pickaxe hasPickaxe(final Player client) {
        final Item wep = client.getEquipment().get(3);
        Pickaxe pick;
        if(wep != null && (pick = pickaxes.get(wep.getId())) != null)
            if(ContentEntity.returnSkillLevel(client, Skills.MINING) >= pick.level)
                return pick;
        for(final Item item : client.getInventory().toArray()){
            if(item != null)
    ...
    public void init() throws FileNotFoundException {
        for(final Rock rock : Rock.values()){
            for(int i = 0; i < rock.rockIds.length; i++)
                rockOres.put(rock.rockIds[i], rock);
        }
        for(final Pickaxe pic : Pickaxe.values()){
    ...
    public int[] getValues(final int type) {
        if(type == 6 || type == 7){
            final List<Integer> oreIds = new ArrayList<>();
            for(final Rock rock : Rock.values()){
                for(int i = 0; i < rock.rockIds.length; i++)
    ...
    @Override
    public boolean clickObject(final Player player, final int type, final int rockId, final int x, final int y, final int d) {
        if(type == 7){
    ...
    private void prospect(final Player player, final int rockId) {
    ...
    public enum Pickaxe {
        ...
        Pickaxe(final int pick, final int level, final int anim, final int value) {
            this.pick = pick;
            this.level = level;
            this.anim = anim;
            this.value = value;
    ...
    public enum Rock {
        ...
        Rock(final int oreId, final int level, final int xp, final int mineTime, final int respawn, final int[] rockIds) {
            this.oreId = oreId;
            this.level = level;
            this.xp = xp;
