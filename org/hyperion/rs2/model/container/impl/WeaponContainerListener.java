...
public class WeaponContainerListener implements ContainerListener {
    ...
    private void sendWeapon() {
        ...
        player.cE.setAtkEmote(WeaponAnimManager.getAttackAnimation(player, id, player.cE.getAtkType()));
        ...
        if(player.debug){
            player.getActionSender().sendMessage(WeaponAnimManager.getSpeed(genericName, id) + "");
        }
        if(shield == null)
            player.cE.setDefEmote(WeaponAnimManager.getDefendAnimation(player, id, -1));
        else
            player.cE.setDefEmote(WeaponAnimManager.getDefendAnimation(player, id, shield.getId()));
        ...
        final Weapon weap = Weapon.forId(id);
    ...
    private void sendWeapon(final int id, final String name, final String genericName) {
        ...
        }else if(name.endsWith("bow") || name.contains("Morr") || name.contains("cannon") || name.startsWith("Crystal bow") || name.startsWith("Toktz-xil-ul") || name.equals("Seercull") || name.endsWith("bow full") || name.contains("bow (class")){
        ...
        }else if(name.startsWith("Staff") || name.endsWith("staff") || name.endsWith("Toktz-mej-tal") || name.endsWith("wand")){
        ...
        }else if(genericName.endsWith("dart") || genericName.endsWith("knife") || genericName.endsWith("thrownaxe") || genericName.equals("Toktz-xil-ul")){
        ...
        }else if(genericName.endsWith("mace") || genericName.endsWith("anchor")){
            player.getActionSender().sendSidebarInterface(0, 3796);
            player.getActionSender().sendInterfaceModel(3797, 200, id);
            player.getActionSender().sendString(3799, name);
        }else if(genericName.contains("maul") || genericName.equals("Tzhaar-ket-om") || genericName.equals("Torags hammers") || genericName.equals("Barrelchest anchor") || genericName.contains("tatius")){
        ...
        }else if(genericName.endsWith("spear") || genericName.endsWith("Guthans warspear")){
    ...
    private String filterWeaponName(String name) {
        final String[] filtered = new String[]{"Iron", "Steel", "Scythe", "Black", "Mithril", "Adamant", "Rune",
                "Granite", "Dragon", "Drag", "Crystal", "Bronze"};
