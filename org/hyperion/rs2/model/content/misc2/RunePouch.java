...
public class RunePouch implements ContentTemplate {
    ...
    private static boolean isRune(final Item item) {
    ...
    public static void withdraw(final Player player, final int id, final int amount) {
        ...
        final Item item = player.getRunePouch().get(slot);
        ...
        if((player.getInventory().freeSlots() <= 0) && (player.getInventory().getById(id) == null)){
    ...
    public static final void deposit(final Player player, final int slot, final int id, final int amount) {
        ...
        try{
            ...
            final Item newItem = (newInventoryAmount <= 0) ? null : new Item(item.getId(), newInventoryAmount);
    ...
    public static void empty(final Player player) {
        if(player == null || player.getRunePouch() == null)
            return;
        for(final Item rune : player.getRunePouch().toArray()){
            if(rune != null){
    ...
    @Override
    public boolean clickObject2(final Player player, final int type, final int a, final int b, final int c, final int d) {
