...
public class Summoning implements ContentTemplate {
    ...
    private static int[] mergeWithplusplusArray(final int[] arr) {
    ...
    public static boolean isBoB(final int npcId) {
        for(final int i : mergeWithplusplusArray(BoBids))
            if(i == npcId)
                return true;
        return false;
    }

    public void addExperience(final Player p, final int amount) {
        p.getSkills().addExperience(23, amount);
    }

    public boolean hasItems(final Player p, final int charmId, final int itemId, final int shardsAmount, final int req) {
        if(ContentEntity.getItemAmount(p, charmId) < 1){
            final ItemDefinition ID = ItemDefinition.forId(charmId);
            p.getActionSender().sendMessage("You need a " + ID.getName() + " to make this pouch.");
            return false;
        }
        if(ContentEntity.getItemAmount(p, itemId) < 1){
            final ItemDefinition ID = ItemDefinition.forId(itemId);
            p.getActionSender().sendMessage("You need " + ID.getName() + " to make this pouch.");
            return false;
        }
        ...
        if(ContentEntity.getItemAmount(p, SHARD) < shardsAmount){
            p.getActionSender().sendMessage("You need " + shardsAmount + " shards to make this pouch.");
            return false;
        }

        if(p.getSkills().getLevel(23) < req){
            p.getActionSender().sendMessage("You need a Summoning Level of " + req + " to make this pouch.");
            return false;
        }
    ...
    public void ItemonItem(final Player p, final int itemUsed, int useWith) {
        int charmId = 1;
        int itemId = 1;
        int shardsAmount = 1;
        final int req;
    ...
    @Override
    public boolean clickObject(final Player player, final int type, final int a, final int b, final int c, final int slot) {
        if(type == 13){
        ...
        if(type == 11){
            final Entity npc = World.getWorld().getNPCs().get(slot);
    ...
    public int[] getValues(final int type) {
        if(type == 13){
            final int[] items = {2138, 2859, 6291, 3369, 440, 6319, 1783, 3095, 12168, 2134, 3138, 6032, 9976, 3325,
                    12156, 1519, 12164, 12165, 12167, 2349, 6010, 12153, 2351, 13403, 1635, 2132, 9978, 12161, 1937,
                    2353, 311, 10099, 10103, 10095, 9736, 7801, 8431, 2359, 2150, 7939, 383, 1963, 1933, 10117, 14616,
                    4188, 6979, 2460, 2361, 10020, 12162, 5933, 1442, 1438, 1440, 571, 6155, 4699, 10149, 2363, 237,
                    1444, 3228, 7979, 1115, 10818, 1119};
            return items;
        }
        if(type == 17){
            final int[] clickitem = {12047, 12043, 12059, 12019, 12009, 12778, 12049, 12055, 12808, 12067, 12091, 12800,
                    12053, 12065, 12021, 12818, 12780, 12814, 12798, 12073, 12087, 12071, 12051, 12095, 12099, 12097,
                    12101, 12103, 12105, 12107, 12816, 12075, 12041, 12061, 12007, 12035, 12027, 12077, 12531, 12810,
                    12812, 12784, 12023, 12085, 12037, 12015, 12045, 12079, 12123, 12031, 12029, 12033, 12820, 12057,
                    14623, 12792, 12069, 12011, 12081, 12782, 12794, 12013, 12802, 12804, 12806, 12025, 12017, 12788,
                    12776, 12083, 12039, 12786, 12089, 12796, 12822, 12093, 12790, 17989, 17988, 17987, 17986, 17985};
            return clickitem;
        }
        if(type == 11){
