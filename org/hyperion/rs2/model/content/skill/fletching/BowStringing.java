...
public class BowStringing extends LogCutting {
    ...
    public static StrungItems getItem(final int i) {
        for(final StrungItems item : StrungItems.values()){
            if(item.getItemId() == i)
                return item;
        }
        return null;
    ...
    public static StringItem getString(final int i) {
        for(final StringItem item : StringItem.values()){
            if(item.getItemId() == i)
                return item;
        }
        return null;
    ...
    public static boolean stringBow(final Player client, final int stringId, final int index) {
        if(client.isBusy()){
            return false;
        }
        final StringItem string = getString(stringId);
        if(string == null)
            return false;
        final StrungItems item = string.getItems()[index];
        if(item == null)
            return false;
    ...
    public enum StringItem {
        BOW_STRING(1777, StrungItems.NORMAL_SHORTBOW, StrungItems.NORMAL_LONGBOW, StrungItems.OAK_SHORTBOW, StrungItems.OAK_LONGBOW, StrungItems.WILLOW_SHORTBOW, StrungItems.WILLOW_LONGBOW, StrungItems.MAPLE_SHORTBOW, StrungItems.MAPLE_LONGBOW, StrungItems.YEW_SHORTBOW, StrungItems.YEW_LONGBOW, StrungItems.MAGIC_SHORTBOW, StrungItems.MAGIC_LONGBOW);

        int itemId;
        StrungItems[] items;

        StringItem(final int itemId, final StrungItems... items) {
            this.itemId = itemId;
            this.items = items;
        }
    ...
    public enum StrungItems {
        ...
        StrungItems(final cutItems startItem, final int resultId, final int emote) {
