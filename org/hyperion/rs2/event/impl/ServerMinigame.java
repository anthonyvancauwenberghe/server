...
public class ServerMinigame extends Event {
    ...
    public static class CountDownEventBuilder {
        ...
        public CountDownEventBuilder(final String name, final String specialArea, final boolean safe) {
            this(name, "::" + specialArea, SpecialAreaHolder.get(specialArea).get(), safe);
        }

        public CountDownEventBuilder(final String name, final String command, final SpecialArea area, final boolean safe) {
            this(name, command, area.getDefaultLocation(), "5x Pk points for 30 minutes", () -> area.createEvent(), safe);
        }

        public CountDownEventBuilder(final String name, final String command, final Location location, final String msg, final Runnable run, final boolean safe) {
            this.name = name;
