...
public class HunterNpcs {
    ...
    public static int getRandomImp() {
        final int max = Hunter.IMP_IDS.length - 1;
        final int r1 = Misc.random(max);
        final int r2 = Misc.random(max);
        final int min = Math.min(r1, r2);
    ...
    public static void spawnImp() {
        if(Server.OLD_SCHOOL)
            return;
        final int x = MIN_X + Misc.random(MAX_X - MIN_X);
        final int y = MIN_Y + Misc.random(MAX_Y - MIN_Y);
        final int impId = getRandomImp();
        final NPC imp = World.getWorld().getNPCManager().addNPC(Location.create(x, y, 0), impId, -1);
        synchronized(imps){
            imps.add(imp);
        }
    }

    public static boolean removeImp(final int NpcId, final int x, final int y) {
        NPC caughtImp = null;
        synchronized(imps){
            for(final NPC imp : imps){
    ...
    public static void randomWalk(final NPC npc) {
        ...
        int walkToY;
        do {
            walkToX = npc.getLocation().getX() + (Combat.random(1) == 0 ? Misc.random(50) : Misc.random(-50));
        }while(walkToX > MAX_X || walkToX < MIN_X);
        do {
            walkToY = npc.getLocation().getY() + (Combat.random(1) == 0 ? Misc.random(50) : Misc.random(-50));
