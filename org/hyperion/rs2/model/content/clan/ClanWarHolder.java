...
public class ClanWarHolder {
    ...
    public void setWar(final ClanWar war) {
        if(this.war != null)
            throw new IllegalStateException("Clan war has already been set!");
        this.war = war;
    }

    public boolean hasDied(final Player player) {
        return hasDied(player.getName());
    }

    public boolean inOriginal(final Player player) {
        return inOriginal(player.getName());
    }
