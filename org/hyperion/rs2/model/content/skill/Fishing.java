...
public class Fishing implements ContentTemplate {
    ...
    public static class FishingFirstClick {
         ...
         */
        private final int[] FISHING_SPOT_NET = {316, 320, 319, 323, 325, 327, 326, 332, 330};
        private final int[] FISHING_SPOT_FLY = {309, 310, 311, 314, 315, 317, 318, 328, 329, 331};
         ...
         * @return
         */
        public final boolean clickObject(final Player client, final int object, final int x, final int y) {

            if(!isFishNet(object) && !isFishLobster(object) && !isFishFly(object) && !isFishTuna(object)){
                return false;
            }
            ...
            if(isFishNet(object)){
                ...
                if(net == -1){
                    ContentEntity.sendMessage(client, "You need a net to fish here.");
                ...
                if(fly == -1){
                    ContentEntity.sendMessage(client, "You need a fly fishing rod to fish here.");
                    client.setBusy(false);
                    client.getExtraData().remove("fishing");
                    return true;
                }
                ...
                if(feather == -1){
                    ContentEntity.sendMessage(client, "You need feathers to fish here.");
                    client.setBusy(false);
                    client.getExtraData().remove("fishing");
                    return true;
                }
                ...
                if(cage == -1){
                    ContentEntity.sendMessage(client, "You need a lobster cage to fish here.");
                    client.setBusy(false);
                    client.getExtraData().remove("fishing");
                    return true;
                }
                ...
                if(harpoon == -1){
                    ContentEntity.sendMessage(client, "You need a harpoon to fish here.");
                    client.setBusy(false);
                    client.getExtraData().remove("fishing");
                    return true;
                }
            ...
            if(ContentEntity.freeSlots(client) == 0){
                ContentEntity.sendMessage(client, "There is not enough space in your inventory.");
                client.getExtraData().remove("fishing");
                return true;
            }
            ...
            if(!(ContentEntity.returnSkillLevel(client, 10) >= level)){
                ContentEntity.sendMessage(client, "You do not have the required level to fish that.");
                client.getExtraData().remove("fishing");
                return true;
            }
                    ...
                    if(cycle == 1 || Math.random() > FISHING_FACTOR){
                        ...
                        if(contains(FISHING_SPOT_NET, object)){
                            ...
                            if(ContentEntity.returnSkillLevel(client, 10) >= 5 && Combat.random(100) + 5 <= ContentEntity.returnSkillLevel(client, 10)){
                                ...
                                ContentEntity.sendMessage(client, "You catch some anchovies.");
                            }else{
                                ContentEntity.addItem(client, 317, 1);
                                ContentEntity.sendMessage(client, "You catch some shrimp.");
                            }
                        }else if(contains(FISHING_SPOT_LOBSTER, object)){
                            if(ContentEntity.returnSkillLevel(client, 10) >= 62 && Combat.random(100) + 62 <= ContentEntity.returnSkillLevel(client, 10)){
                                ...
                                ContentEntity.sendMessage(client, "You catch a Monkfish.");
                            }else{
                                xp = 100;
                                ContentEntity.addItem(client, 377, 1);
                                ContentEntity.sendMessage(client, "You catch a lobster.");
                            }
                        }else if(contains(FISHING_SPOT_TUNA, object)){
                            xp = 80;
                            ContentEntity.addItem(client, 359, 1);
                            ContentEntity.sendMessage(client, "You catch a tuna.");
                        }else if(contains(FISHING_SPOT_FLY, object)){
                            if(ContentEntity.isItemInBag(client, FEATHER[0])){
                                if(ContentEntity.returnSkillLevel(client, 10) >= 30){
                                    if(Math.random() < 0.45){
                                        xp = 70;
                                        ContentEntity.addItem(client, 331, 1);
                                        ContentEntity.sendMessage(client, "You catch a salmon.");
                                        ContentEntity.deleteItem(client, 314, FEATHERSLOT, 1);
                                    }else{
                                        xp = 50;
                                        ContentEntity.addItem(client, 335, 1);
                                        ContentEntity.sendMessage(client, "You catch a trout.");
                                        ContentEntity.deleteItem(client, 314, FEATHERSLOT, 1);
                                    }
                                ...
                                ContentEntity.sendMessage(client, "You need a feather to fish here.");
                                stop2();
                            }
                        }
                        // add xp
                        ContentEntity.addSkillXP(client, xp * EXPMULTIPLIER, 10);
                    }
        ...
        public int getFishLevel(final int id) {
            int level = 1;
         ...
         * @return
         */
        public boolean isFishNet(final int id) {
         ...
         * @return
         */
        public boolean isFishFly(final int id) {
            return contains(FISHING_SPOT_FLY, id);
        }

        public boolean checkFeather(final Player client) {
            for(final int id : FEATHER){
         ...
         * @return
         */
        public boolean isFishLobster(final int id) {
         ...
         * @return
         */
        public boolean isFishTuna(final int id) {
         ...
         * @return
         */
        private final int hasNet(final Player client) {
            int ct = 0;
            for(final int id : FISHING_NET){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
                }
                ct++;
        ...
        private final int getFeatherSlot(final Player client) {
            return ContentEntity.getItemSlot(client, 314);
        }

        private final int hasFly(final Player client) {
            int ct = 0;
            for(final int id : FLY_ROD){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
                }
                ct++;
        ...
        private final int hasFeather(final Player client) {
            int ct = 0;
            for(final int id : FEATHER){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
        ...
        private final int hasCage(final Player client) {
            int ct = 0;
            for(final int id : LOBSTER_POT){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
                }
                ct++;
        ...
        private final int hasHarpoon(final Player client) {
            int ct = 0;
            for(final int id : HARPOON){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
                }
                ct++;
    ...
    public class FishingSecondClick {
         ...
         * A huge collection of constants here.
         */
        private final int[] FISHING_SPOT_BAIT = {316, 320, 319, 323, 325, 327, 326, 332, 330};
        ...
        public final boolean clickObject(final Player client, final int object, final int x, final int y) {
            ...
            if(isFishBait(object)){
                ...
                if(rod == -1){
                    ContentEntity.sendMessage(client, "You need a rod to fish here.");
                    client.setBusy(false);
                    return true;
                }
                ...
                if(bait == -1){
                    ContentEntity.sendMessage(client, "You need bait to fish here.");
                    client.setBusy(false);
                    return true;
                }
                    ...
                    if(cycle == 1 || Math.random() > FISHING_FACTOR){
                        ...
                        if(contains(FISHING_SPOT_BAIT, object)){
                            if(ContentEntity.isItemInBag(client, 313)){
                                ...
                                ContentEntity.sendMessage(client, "You catch a sardine.");
                                ContentEntity.deleteItem(client, 313, BAITSLOT, 1);
                            }else{
                                ContentEntity.sendMessage(client, "You need bait to fish here.");
                                stop2();
                            }
                            ...
                            if(ContentEntity.returnSkillLevel(client, 10) >= 76 && Combat.random(100) + 76 <= ContentEntity.returnSkillLevel(client, 10)){
                                ...
                                ContentEntity.sendMessage(client, "You catch a shark.");
                            }else{
                                xp = 80;
                                ContentEntity.addItem(client, 371, 1);
                                ContentEntity.sendMessage(client, "You catch a Swordfish.");
                            }
                        }
                        // add xp
                        ContentEntity.addSkillXP(client, xp * EXPMULTIPLIER, 10);
         ...
         * @return
         */
        public boolean isFishBait(final int id) {
            return contains(FISHING_SPOT_BAIT, id);
        }

        public boolean checkBait(final Player client) {
            for(final int id : BAIT){
                if(ContentEntity.isItemInBag(client, id)){
                    return true;
                }
         ...
         * @return
         */
        public boolean isFishShark(final int id) {
            return contains(FISHING_SPOT_HARPOON_SHARK, id);
        }

        private final int getBaitSlot(final Player client) {
            return ContentEntity.getItemSlot(client, 313);

        }

        private final int hasRod(final Player client) {
            int ct = 0;
            for(final int id : BAIT_ROD){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
                }
                ct++;
        ...
        private final int hasBait(final Player client) {
            int ct = 0;
            for(final int id : BAIT){
                if(ContentEntity.isItemInBag(client, id)){
                    return ct;
