...
public class LogEntry implements Comparable<LogEntry> {
    ...
    public static LogEntry privateChat(final String from, final String to, final String msg) {
        return new LogEntry(Category.PRIVATE_CHAT, String.format("@red@%s@blu@ -> @red@%s@blu@: @bla@%s", from, to, msg));
    }
    ...
    public static LogEntry duel(final Player player1, final Player player2) {
        ...
        return new LogEntry(Category.DUEL, String.format("@red@%s (%s)@blu@ vs @red@%s (%s)@blu@%s@red@%s@blu@ Stake: @bla@%s@blu@%s@red@%s@blu@ Stake: @bla@%s", player1.getName(), player1Weapon != null ? player1Weapon.getDefinition().getName() : "UNARMED", player2.getName(), player2Weapon != null ? player2Weapon.getDefinition().getName() : "UNARMED", LogUtils.NEW_LINE, player1.getName(), LogUtils.toString(player1Stake), LogUtils.NEW_LINE, player2.getName(), LogUtils.toString(player2Stake)));
    }
    ...
    public static LogEntry duelResult(final Player winner, final Player loser) {
        ...
        return new LogEntry(Category.DUEL, String.format("@red@%s (Wep: %s, Ammy: %s, Ring: %s)@blu@ vs @red@%s (Wep: %s, Ammy: %s, Ring: %s)@blu@: Winner: @red@%s", winner.getName(),
                //                        winnerHp,
                winnerWeapon != null ? winnerWeapon.getDefinition().getName() : "UNARMED", winnerAmulet != null ? winnerAmulet.getDefinition().getName() : "NONE", winnerRing != null ? winnerRing.getDefinition().getName() : "NONE", loser.getName(),
                //                        loserHp,
                loserWeapon != null ? loserWeapon.getDefinition().getName() : "UNARMED", loserAmulet != null ? loserAmulet.getDefinition().getName() : "NONE", loserRing != null ? loserRing.getDefinition().getName() : "NONE", winner.getName()));
    }

    public static LogEntry trade(final String player1, final String player2, final Item[] player1Trade, final Item[] player2Trade) {
        return new LogEntry(Category.TRADE, String.format("@red@%s@blu@ <> @red@%s@blu@%s@red@%s@blu@ Trade: @bla@%s%s@red@%s@blu@ Trade: @bla@%s", player1, player2, LogUtils.NEW_LINE, player1, LogUtils.toString(player1Trade), LogUtils.NEW_LINE, player2, LogUtils.toString(player2Trade)));
    }

    public static LogEntry bob(final String player1, final Item[] bobItems) {
        return new LogEntry(Category.BOB, String.format("@red@%s@blu@ Bob Drops: @bla@%s", player1, LogUtils.toString(bobItems)));
    }
    ...
    public static LogEntry death(final Player player, final Player killer, final Item[] items) {
        return new LogEntry(Category.DEATH, String.format("@blu@Killed By: @red@%s (ip match: %s, mac match: %s)%s@blu@Lost:@bla@ %s", (killer != null ? killer.getName() : "---"), Objects.equals(player.getShortIP(), killer != null ? killer.getShortIP() : ""), player.getUID() == (killer != null ? killer.getUID() : -1), LogUtils.NEW_LINE, LogUtils.toString(items)));
    }

    public static LogEntry kill(final Player killer, final Player player, final Item[] items) {
        return new LogEntry(Category.KILL, String.format("@blu@Killed: @red@%s (ip match: %s, mac match: %s)%s@blu@Lost:@bla@ %s", (player != null ? player.getName() : "---"), Objects.equals(killer.getShortIP(), player != null ? player.getShortIP() : ""), killer.getUID() == (player != null ? player.getUID() : -1), LogUtils.NEW_LINE, LogUtils.toString(items)));
    }

    public static LogEntry pickupItem(final GlobalItem item) {
        return new LogEntry(Category.PICKUP_ITEM, String.format("%s [Owner: %s]", LogUtils.toString(item.getItem()), item.owner == null ? "null" : item.owner.getName()));
    }
    ...
    public static LogEntry gamble(final NPC npc, final Item item, final int n) {
        return new LogEntry(Category.GAMBLE, String.format("%s - %s (%d) Rolled %d (%s)", LogUtils.toString(item), npc.getDefinition().getName(), npc.getDefinition().getId(), n, n >= 55 ? "Won" : "Lost"));
    }

    public static LogEntry geEntryAdded(final Entry entry) {
        return new LogEntry(Category.GRAND_EXCHANGE, String.format("Slot %d - %s %,d %s (%d) @ %,d %s", entry.slot, entry.type, entry.itemQuantity, entry.item().getDefinition().getName(), entry.itemId, entry.unitPrice, entry.currency));
    }

    public static LogEntry geProgress(final Progress progress) {
        return new LogEntry(Category.GRAND_EXCHANGE, String.format("%s %,d by %s @ %,d each", progress.type, progress.quantity, progress.playerName, progress.unitPrice));
    }
