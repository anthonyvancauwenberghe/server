...
public class SkillSetCommand extends Command {
     ...
     * to their skill id.
     */
    private static final String[][] COMMAND_NAMES = {{"atk", "attk", "attack"}, {"def", "defense", "defence"},
            {"str", "strength"}, {"hp", "hitp", "hitpoints"}, {"range", "ranging", "ranged"}, {"pray", "prayer"},
            {"mage", "magic"}};
    /**
     ...
     * @param skill
     */
    private SkillSetCommand(final String startsWith, final int skill, final Rank... rights) {
     ...
     * @return
     */
    public static boolean canChangeLevel(final Player player) {
        if(player.getLocation().inPvPArea()){
            player.getActionSender().sendMessage("You cannot use this command in PvP zones.");
            return false;
    ...
    public static void init() {
        for(int i = 0; i < COMMAND_NAMES.length; i++){
            for(final String name : COMMAND_NAMES[i]){
