...
public class DungeoneeringManager implements ContentTemplate {
    ...
    @Override
    public boolean clickObject2(final Player player, final int type, final int npcId, final int x, final int y, final int npcSlot) {
    ...
    public boolean dialogueAction(final Player player, final int dialogueId) {
        switch(dialogueId){
            ...
            case 7001:
                ...
                break;
            case 7002:
                player.getActionSender().sendDialogue("Leave?", ActionSender.DialogueType.OPTION, 1, Animation.FacialAnimation.DEFAULT, "Yes, I want to leave this dungeon", "No, I don't want to lose my progress");
            ...
            case 7010:
                ...
                break;
            case 7011:
                player.getActionSender().sendDialogue("Rewards Trader", ActionSender.DialogueType.OPTION, 9711, Animation.FacialAnimation.DEFAULT, "Dungeoneering Guide", "What are perks?", "Upgrade ring perks");
            ...
            case 7012:
                ...
                break;
            case 7013:
                player.getActionSender().sendDialogue("Rewards Trader", ActionSender.DialogueType.NPC, 9711, Animation.FacialAnimation.DEFAULT, "Perks are bonuses that your ring of kinship possesses (when worn)", "They work both inside and outside dungeoneering", "They cost dungeoneering tokens to upgrade");
            ...
            case 7019:
                final String[] perk_pricse = Stream.of(RingPerks.Perk.values()).
                        map(perk -> perk.maxLevel != player.getDungeoneering().perks.hasPerk(perk) ? String.format("%s %d tokens", perk.name(), player.getDungeoneering().perks.calcNextPerkCost(perk.index)) : String.format("%s MAXED", perk.name())).toArray(String[]::new);
                player.getActionSender().sendDialogue("Rewards Trader", ActionSender.DialogueType.OPTION, 9711, Animation.FacialAnimation.DEFAULT, perk_pricse);
                for(int i = 0; i < 3; i++)
