...
public class Skills {
     ...
     */
    public static final int ATTACK = 0, DEFENCE = 1, STRENGTH = 2, HITPOINTS = 3, RANGED = 4, PRAYER = 5, MAGIC = 6, COOKING = 7, WOODCUTTING = 8, FLETCHING = 9, FISHING = 10, FIREMAKING = 11, CRAFTING = 12, SMITHING = 13, MINING = 14, HERBLORE = 15, AGILITY = 16, THIEVING = 17, SLAYER = 18, FARMING = 19, RUNECRAFTING = 20, CONSTRUCTION = 21, HUNTER = 22, SUMMONING = 23, DUNGEONEERING = 24;
    /**
     * The bonus skill.
     */
     ...
     */
    public static int[] EXPERIENCE_PER_LEVEL = {-1, 0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, 1358, 1584, 1833,
            2107, 2411, 2746, 3115, 3523, 3973, 4470, 5018, 5624, 6291, 7028, 7842, 8740, 9730, 10824, 12031, 13363,
            14833, 16456, 18247, 20224, 22406, 24815, 27473, 30408, 33648, 37224, 41171, 45529, 50339, 55649, 61512,
            67983, 75127, 83014, 91721, 101333, 111945, 123660, 136594, 150872, 166636, 184040, 203254, 224466, 247886,
     ...
     */
    private final boolean[] changedLevels = new boolean[SKILL_COUNT];
    /**
    ...
    public static boolean isCombatSkill(final int id) {
        switch(id){
            case 0:
            case 1:
            case 2:
    ...
    public int getCombatLevel() {
        return (int) Math.floor((0.25 * ((getLevelForExp(DEFENCE)) + (getLevelForExp(HITPOINTS)) + Math.floor((getLevelForExp(PRAYER)) / 2))) +
                /*(0.125 * getLevelForExp(23)) +*/
                Math.max((0.325 * ((getLevelForExp(ATTACK)) + (getLevelForExp(STRENGTH)))), Math.max((0.325 * (Math.floor((getLevelForExp(RANGED)) / 2) + (getLevelForExp(RANGED)))), (0.325 * (Math.floor((getLevelForExp(MAGIC)) / 2) + (getLevelForExp(MAGIC)))))));
    }
    ...
    public void incrementLevel(final int skill, int amount) {
        if(levels[skill] == 0){
            amount = 0;
        }
    ...
    public void normalizeLevel(final int skill) {
        final int norm = getLevelForExp(skill);
    ...
    public void speedNormalizeLevel(final int skill) {
        final int norm = getLevelForExp(skill);
        if(levels[skill] > norm){
            levels[skill] = norm;
            player.getActionSender().sendSkill(skill);
    ...
    public int getExperienceTilLevel(final int skill, final int targetLevel) {
    ...
    public void addExperience(final int skill, double exp) {
            ...
            synchronized(World.getWorld()){
                ...
                if(dayOfYear != lastSkillChange){
                    if(bonusSkill != BONUS_SKILL){
                        ...
                        for(final Player player1 : World.getWorld().getPlayers()){
        ...
        if(skill == BONUS_SKILL)
            ...
            exp *= 2;
        resetBonuxXP();
        final int oldLevel = getLevelForExp(skill);
        final int oldExp = getExperience(skill);
        exps[skill] += exp;
        ...
        final int newLevel = getLevelForExp(skill);
     ...
     * @return
     */
    public boolean hasChanged(final int skill) {
