...
public class PlayerDeathEvent extends Event {
    ...
    private static final String[] KILLER_MESSAGES = new String[]{"You have wiped the floor with %s.",
            "%s regrets the day that he met you.", "You rock, clearly %s does not.", "You have sent %s to his grave.",
            "All the kings horses and men could never put %s back together again...", "%s falls before your might.",
            "With a crushing blow %s's life is met with an untimely end.", "You have ended %s's life abruptly.",
            "The mysteries of life can no longer be discovered by %s.", "The sword is obviously mightier than %s.",
            "The death of %s is a burden you must bear.", "%s must of dissapointed the gods...",
            "I think %s said square root, not square up - either way, he died."};
    private static final String[] DEATH_MESSAGES = new String[]{"Oh dear! You are dead", "Death is a harsh mistress",
            "Life is fragile, you had to learn it the hard way", "Life is part of a cycle, the cycle just ended.",
            "The darkness of the afterlife awaits you...", "You're stupid... and dead"};
    ...
    private static void handlePkpTransfer(final Player killer, final Player player, final int original) {
        int toTransfer = pkpToTransfer(player);
        final PlayerPoints kP = player.getPoints();
        if(kP.getPkPoints() < toTransfer){
            final int deltaRemove = (toTransfer - kP.getPkPoints() + 10) / 9;
            final int remove = player.getBank().remove(Item.create(5020, deltaRemove));
            ...
            if(!player.getLocation().inFunPk() && !LastManStanding.inLMSArea(player.cE.getAbsX(), player.cE.getAbsY())){
                if(killer != null){
                    ...
                    if(true || killer.getLocation().inPvPArea()){
                        boolean isDev = false;
                        if(Rank.getPrimaryRank(killer).ordinal() >= Rank.DEVELOPER.ordinal() || Rank.getPrimaryRank(player).ordinal() >= Rank.DEVELOPER.ordinal())
                            isDev = true;
                        if(!isDev){
                            killer.increaseKillCount();
                            final int oldKillerRating = killer.getPoints().getEloRating();
