...
public class Prayer implements ContentTemplate {
    ...
    public static int getBonesExp(final int id) {
    ...
    public static void retribution(final Player p) {
        ...
        if(p.getPrayers().isEnabled(47)){
            ...
            for(final Player player : p.getLocalPlayers()){
    ...
    public static void redemption(final Player p) {
        if(p.getPrayers().isEnabled(22)){
            if(p.getSkills().getLevel(3) < (p.getSkills().getLevelForExp(3) / 10) && p.getSkills().getLevel(3) > 0){
                p.cE.doGfx(436, 0);
    ...
    public static void smite(final Player p, double hit) {
        hit = (hit / 4);
        p.getSkills().detractLevel(5, (int) hit);
    }

    public static void soulSplit(final Player player, final CombatEntity victim, final int damg) {
        ...
        if(victim.getEntity() instanceof Player){
            final int amount = (int) (damg * 0.2);// 40%
            victim.getPlayer().getSkills().detractLevel(5, amount);
            if(!player.isDead())
                player.heal(amount);
        }else{
            final int amount = (int) (damg * 0.2);// 20%
            if(!player.isDead())
                player.heal(amount);
        }
            ...
            public void execute() {
                try{
                    ...
                    if(victim.getEntity() instanceof Player && distance < 20){
                        final int offsetY = (victim.getAbsX() - player.cE.getAbsX()) * -1;
                        final int offsetX = (victim.getAbsY() - player.cE.getAbsY()) * -1;
    ...
    public static void zaniksEffect(final Player player, final int damage) {
        ...
        player.getCombat().getOpponent()._getPlayer().ifPresent(opp -> {
            ...
            for(final int i : Prayers.OVERHEADS){
                ...
                if(opp.getPrayers().isEnabled(i)){
                    ...
                    if(i == -1)
                        continue;
                    final PrayerIcon p2 = prayers.get(idx);
    ...
    public static void dragonScimitar(final Player player) {
    ...
    public static void resetInterface(final Player player) {
        if(prayers == null)
            return;
        for(final PrayerIcon prayer : prayers){
    ...
    public static int getIndex(final int item) {
        for(final PrayerIcon p : prayers){
            if(p.getId() == item)
    ...
    public static double getPryDrain(final Player c) {
    ...
    public static void changeCurses(final Player player) {
        ...
        final boolean current_book = player.getPrayers().isDefaultPrayerbook();
    ...
    public static void setHeadIcon(final Player c, final PrayerIcon icon) {
        c.headIconId = getIconId(c, icon);
        c.getUpdateFlags().flag(UpdateFlag.APPEARANCE);
    }

    public static void setHeadIcon(final Player p) {
        setHeadIcon(p, null);
    }

    public static int getIconId(final Player p, final PrayerIcon icon) {
        // curses
    ...
    private static int usingOnAltar(final int objectid) {
        switch(objectid){
            case 409:
                return 2;
            case 13192:
    ...
    public static boolean isPrayerForSkillIdOnDrain(final Player player, final int index) {
        if(index == 0 && (player.getPrayers().isEnabled(31) || player.getPrayers().isEnabled(40) || player.getPrayers().isEnabled(49)))// att
            return true;
        if(index == 1 && (player.getPrayers().isEnabled(31) || player.getPrayers().isEnabled(43) || player.getPrayers().isEnabled(49)))// def
            return true;
        if(index == 2 && (player.getPrayers().isEnabled(31) || player.getPrayers().isEnabled(44) || player.getPrayers().isEnabled(49)))// str
            return true;
    ...
    public boolean buryBones(final Player player, final int fromSlot, final int item, final boolean altar, final int multiplier) {
        final int buryItem = item;
        int buryXP = getBonesExp(buryItem);
        buryXP *= multiplier;
        if(player.getEquipment().get(Equipment.SLOT_AMULET) != null && player.getEquipment().get(Equipment.SLOT_AMULET).getId() == 1718){
            buryXP *= 2;
        }
        if(buryXP == -1){
            return false;
    ...
    public void rechargePrayer(final Player player) {
        player.playAnimation(Animation.create(645, 0));
        player.getSkills().setLevel(5, player.getSkills().getLevelForExp(5));
     ...
     */
    @Override
    public boolean clickObject(final Player p, final int type, int id, final int itemSlot, final int c, final int d) {
        ...
        if(getIndex(id) == -1)
            return false;
        final PrayerIcon p2 = prayers.get(getIndex(id));


        for(final Iterator<Prayers.DisabledPrayer> it = p.getPrayers().getDisabled().iterator(); it.hasNext(); ){
            final Prayers.DisabledPrayer disabledPrayer = it.next();
            final long disabledFor = System.currentTimeMillis() - disabledPrayer.getPrayersDisabledAt();
            if(disabledFor <= disabledPrayer.getPrayersDisabledFor()){
                final long timeRemaining = (disabledPrayer.getPrayersDisabledFor() - disabledFor) / 1000;
                for(final int prayer : disabledPrayer.getPrayersDisabled()){
                    if(prayer == p2.getId()){
        ...
        if(p2.isCurse() && p.getSkills().getLevelForExp(Skills.DEFENCE) < 30){
        ...
        if(p.getSkills().getLevelForExp(1) < 70 && p2.getName().equals("Piety")){
            p.getActionSender().sendClientConfig(p2.getFrame(), 0);
            p.getActionSender().sendMessage("You need a Defence level of 70 to use " + p2.getName() + ".");
            p.getActionSender().sendString(357, "You need a @dbl@Defence level of 70 to use " + p2.getName() + ".");
            p.getActionSender().sendPacket164(356);
            return true;
        }
        if(p.getSkills().getLevelForExp(1) < 60 && p2.getName().equals("Chivalry")){
            p.getActionSender().sendClientConfig(p2.getFrame(), 0);
            p.getActionSender().sendMessage("You need a Defence level of 60 to use " + p2.getName() + ".");
            p.getActionSender().sendString(357, "You need a @dbl@Defence level of 60 to use " + p2.getName() + ".");
            p.getActionSender().sendPacket164(356);
            return true;
        }
        ...
        if(!hasUnlocked(p, p2.getId())){
            p.getActionSender().sendClientConfig(p2.getFrame(), 0);
            p.getActionSender().sendMessage("You haven't unlocked " + p2.getName() + ".");
            p.getActionSender().sendString(357, "You haven't unlocked " + p2.getName() + ".");
            p.getActionSender().sendPacket164(356);
            return true;
        }
        ...
        if(p.getPrayers().isEnabled(p2.getId())){
            ...
            for(int j = 0; j < p2.getPrayers().length; j++){
                if((p2.getPrayers()[j] != -1) && (p.getPrayers().isEnabled(p2.getPrayers()[j]))){
                    final PrayerIcon p1 = prayers.get(getIndex(p2.getPrayers()[j]));
                    // Set the config to 0.
    ...
    void changeAncients(final Player player) {
    ...
    public void changeLunars(final Player player) {
        player.cE.setAutoCastId(-1);
        player.playAnimation(Animation.create(645));
    ...
    public int[] getValues(final int type) {
        if(type == 1){
            final int[] j = new int[BONESEXP.length];
            for(int i = 0; i < BONESEXP.length; i++){
                j[i] = BONESEXP[i][0];
            }
            return j;
        ...
        if(type == 6){
            final int[] j = {61, 409, 410, 411, 412, 2478, 2479, 2480, 2481, 2482, 2483, 2484, 2485, 2486, 2487, 2488,
                    2489, 2490, 2640, 4008, 6552,};
            return j;
        }
        if(type == 14){
            final int[] j = new int[BONESEXP.length];
		 ...
		 */
        final int[] ai = {5609, 5610, 5611, 18000, 18002, 5612, 5613, 5614, 5615, 5616, 5617, 18004, 18006, 5618, 5619,
                5620, 5621, 5622, 5623, 18008, 18010, 683, 684, 685, 18012, 18014, 18016, 18018,
                /* curses */22503, 22505, 22507, 22509, 22511, 22513, 22515, 22517, 22519, 22521, 22523, 22525, 22527,
                22529, 22531, 22533, 22535, 22537, 22539, 22541,};
        return ai;
    }
    ...
    public int getPrayerForActionButton(final int id) {
        final int[] j = {5609, 5610, 5611, 18000, 18002, 5612, 5613, 5614, 5615, 5616, 5617, 18004, 18006, 5618, 5619,
                5620, 5621, 5622, 5623, 18008, 18010, 683, 684, 685, 18012, 18014, 18016, 18018};
        /*
