...
public class Smithing implements ContentTemplate {
    ...
    public final int SMITHING_FRAME[][][] = {
            /* Bronze */{{1205, 1, 1, 1, 1125, 1094}, {1351, 1, 1, 1, 1126, 1091}, {1422, 1, 2, 1, 1129, 1093},
            ...
            /* Iron */{{1203, 1, 15, 1, 1125, 1094}, {1349, 1, 16, 1, 1126, 1091}, {1420, 1, 17, 1, 1129, 1093},
            ...
            /* Steel */{{1207, 1, 30, 1, 1125, 1094}, {1353, 1, 31, 1, 1126, 1091}, {1424, 1, 32, 1, 1129, 1093},
			...
			/* Mithril */{{1209, 1, 50, 1, 1125, 1094}, {1355, 1, 51, 1, 1126, 1091}, {1428, 1, 52, 1, 1129, 1093},
			...
			/* Adamant */{{1211, 1, 70, 1, 1125, 1094}, {1357, 1, 71, 1, 1126, 1091}, {1430, 1, 72, 1, 1129, 1093},
			...
			/* Rune */{{1213, 1, 85, 1, 1125, 1094}, {1359, 1, 86, 1, 1126, 1091}, {1432, 1, 87, 1, 1129, 1093},
    ...
    private static boolean repairChaotics(final Player p, final int id) {
    ...
    public boolean smithItem(final Player client, final int itemId, final int slot, int interfaceId, final int amm2) {
        ...
        if(ContentEntity.returnSkillLevel(client, 13) < SMITHING_FRAME[type][k][2]){
            ContentEntity.sendMessage(client, "You need a smithing level of " + SMITHING_FRAME[type][k][2] + " to make this item.");
            return true;
        }
        ...
        World.getWorld().submit(new Event(2500) {
            ...
            public void execute() {
                ...
                if(ContentEntity.getItemAmount(client, getBar(type)) >= SMITHING_FRAME[type][k][3]){
                    ...
                    if(amm > 1)
                        ContentEntity.startAnimation(client, 898);
                    ContentEntity.addSkillXP(client, SMITHING_FRAME[type][k][2] * 50 * EXPMULTIPLIER, Skills.SMITHING);
                    /*
                     * World.getWorld().submit(new Event(2000) {
					 *
					 * @Override public void execute() {
					 */
                    ContentEntity.deleteItemA(client, getBar(type), SMITHING_FRAME[type][k][3]);
                    ContentEntity.addItem(client, itemId, SMITHING_FRAME[type][k][1]);
    ...
    public int getInterfaceId(final int interfaceId) {
        switch(interfaceId){
            case 57220:
                return 1119;
            case 57476:
    ...
    public int itemExists(final int id, final int menu) {
    ...
    public int getType(final int bar) {
        switch(bar){
    ...
    public int getBar(final int bar) {
        switch(bar){
            case 0:
                return 2349;
            case 1:
    ...
    public void openSmithingFrame(final Player c, final int type) {
        final int realType = type - 1;
        ...
        if(type == 2 || type == 3){
            if(SMITHING_FRAME[realType].length >= 23){
                ...
                final String itemName = ContentEntity.getItemName(SMITHING_FRAME[realType][22][0]);
        ...
        if(type == 3){
            ...
            final String itemName = ContentEntity.getItemName(SMITHING_FRAME[realType][23][0]);
    ...
    public int[] getValues(final int type) {
        if(type == 14){
            final int[] j = {2349, 2351, 2353, 2359, 2361, 2363, 11702, 11704, 11706, 11708, 1540, 18350, 18352, 18354,
                    18356, 18358, 18360};
            return j;
        }
        if(type == 2 || type == 3 || type == 4){
            final int[] j = new int[140];
            int index = 0;
            for(int i = 0; i < 6; i++){
        ...
        if(type == 13){
            final int[] j = {13734, 13736, 13754, 13746, 13748, 13750, 13752,};
            return j;
        }
        return null;
    }
    ...
    private boolean smithSpiritShield(final Player client, int id, int itemId2) {
    ...
    private boolean smithExtraItems(final Player player, final int id) {
        final int[] hiltIds = {11702, 11704, 11706, 11708, 1540,};
        final int[] finishItem = {11694, 11696, 11698, 11700, 11283,};
        final int[][] extraItemIds = {{11710, 11712, 11714,}, {11710, 11712, 11714,}, {11710, 11712, 11714,},
                {11710, 11712, 11714,}, {11286,},// dfs
        };
        final int[] level = {80, 80, 80, 80, 85};
        int index = -1;
        ...
        if(ContentEntity.returnSkillLevel(player, Skills.SMITHING) < level[index]){
            ContentEntity.sendMessage(player, "You need " + level[index] + " to smith this item.");
            return false;
        }
        if(true)
            return false;
        ...
        ContentEntity.sendMessage(player, "You smith a " + ItemDefinition.forId(finishItem[index]).getName() + ".");
        ContentEntity.startAnimation(player, 898);
        ContentEntity.addSkillXP(player, 4000 * EXPMULTIPLIER, Skills.SMITHING);
