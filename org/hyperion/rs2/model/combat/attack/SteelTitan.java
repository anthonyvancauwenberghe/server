...
public class SteelTitan implements Attack {
    ...
    public int handleAttack(final NPC n, final CombatEntity attack) {
        ...
        if(distance > 2 && distance < 8){
            final int tempDamage = CombatCalculation.getCalculatedDamage(n, attack.getEntity(), Combat.random(MAX_RANGE_DAMG), Constants.RANGE, MAX_RANGE_DAMG);
            Combat.npcAttack(n, attack, tempDamage, 1700, Constants.RANGE);
        }else if(distance < 8){
            final int tempDamage = CombatCalculation.getCalculatedDamage(n, attack.getEntity(), Combat.random(MAX_MELEE_DAMG), Constants.MELEE, MAX_MELEE_DAMG);
