...
public class Challenge {
    ...
    public void send(final Player player, final boolean alert) {
        if(alert){
            player.sendf("Alert##%s's Challenge##::challenge %s##Prize: %s x%,d!", getName(), getText(), getPrize().getDefinition().getName(), getPrize().getCount());
        }else{
            player.sendf("@red@----------------------------------------------------------------------------------------");
    ...
    public boolean equals(final Object o) {
        ...
        return c.name.equals(name) && c.text.equals(text) && c.item.getId() == item.getId() && c.item.getCount() == item.getCount();
    }
}
