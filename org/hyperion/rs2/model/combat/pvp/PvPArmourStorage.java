...
public class PvPArmourStorage {
    ...
    private static final int[] armours = new int[]{STATIUS_BODY, STATIUS_LEGS, STATIUS_HELM, STATIUS_WARHAMMER,
            VESTA_BODY, VESTA_LEGS, VESTA_SWORD, MORRIGANS_TOP, MORRIGANS_CHAPS, MORRIGANS_COIF, ZURIELS_BOTTOMS,
            ZURIELS_STAFF, ZURIELS_TOP, ZURIELS_HAT, VESTA_DEG_TOP, VESTA_DEG_LEGS};
    ...
    public static void main(final String[] args) {
        final PvPArmourStorage storage = new PvPArmourStorage();
        System.out.println(storage);
        for(int i = 0; i < 999; i++){
            final int degrade = storage.degrade(STATIUS_HELM);
    ...
    private PvPArmourStorage edit(final int[] armours, final int[] args) {
        for(int i = 0; i < armours.length; i++){
    ...
    public PvPArmourStorage editFromString(final String fromString) {
        final String armourData[] = fromString.split(",");
        final int[] armours = new int[armourData.length];
        final int[] armourCharges = new int[armourData.length];
    ...
    public synchronized boolean contains(final int id) {
        return wrappedMap.containsKey(id);
    }

    public synchronized int degrade(final int id) {
        if(!contains(id))
            return DEFAULT_CHARGE;
        final int amount = wrappedMap.get(id) - 1;
