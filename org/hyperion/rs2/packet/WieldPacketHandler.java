...
public class WieldPacketHandler implements PacketHandler {
    ...
    public static void wearItem(final Player player, final int slot) {
        final int id = player.getInventory().get(slot).getId();
        ...
        if(slot >= 0 && slot < Inventory.SIZE){
            ...
            if(item != null && item.getId() == id){
                ...
                Item oldEquip = null;
                final boolean stackable = ItemDefinition.forId(id).isStackable();
        ...
        switch(interfaceId){
            case Inventory.INTERFACE:
                ...
                if(slot >= 0 && slot < Inventory.SIZE){
                    ...
                    if(item != null && item.getId() == id){
                        final EquipmentType type = Equipment.getType(item); //OMG FOUNDS THIS SHIT
                        ...
                        if(oldEquip != null){
                            if(oldEquip.getId() == item.getId() && stackable){
                                final long totalEquip = oldEquip.getCount() + item.getCount();
