...
public class TeleTabs implements ContentTemplate {
    ...
    public static void TeleTab(final Player player, int x, int y, final int z, final int id) {
        x += Combat.random(1);
        y += Combat.random(1);
    ...
    @Override
    public boolean clickObject(final Player p, final int type, final int a, final int b, final int c, final int d) {
        if(type == 1){
            switch(a){
                case 8007:
    ...
    public int[] getValues(final int type) {
        if(type == 1){
            final int[] j = {8007, 8008, 8009, 8010, 8011, 8012, 18806,};
            return j;
        }
        return null;
    }
