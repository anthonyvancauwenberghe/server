...
public class DefaultAttack implements Attack {
    ...
    public int handleAttack(final NPC n, CombatEntity attack) {
        if(Location.create(n.getLocation().getX() + n.cE.getOffsetX(), n.getLocation().getY() + n.cE.getOffsetY(), n.getLocation().getZ()).isWithinDistance(n.cE.getOpponent().getEntity().getLocation(), 1 + ((n.getDefinition().sizeX() + n.getDefinition().sizeY()) / 2))){
            ...
            final int tempDamage = CombatCalculation.getCalculatedDamage(n, attack.getEntity(), Combat.random(maxHit), Constants.MELEE, maxHit);
            ...
            final int distance = n.getLocation().distance(n.cE.getOpponent().getEntity().getLocation());
