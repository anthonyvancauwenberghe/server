...
public class NPCDefinition {
    ...
    private static final NPCDefinition[] definition = new NPCDefinition[12000];
    //public int[][] dropId = null;//structer dropId[][itemId-0,minAm-1, maxAm-2, percentage out of 1000-3]
    //Map.Entry<NPCID, NPCDrop>
    private final List<NPCDrop> drops = new LinkedList<>(); //too lazy to find/set size cba
    private final boolean doesDefEmote = true;
    /**
     * The id.
     */
     ...
     * @param id The id.
     */
    public NPCDefinition(final int id) {
        this.id = id;
    }

    public NPCDefinition(final int id, final int maxHp, final int cb, final int[] bonus, final int deathAnim, final int blockAnim, final int[] atkAnims, final int size, final String name, final int spawnTime) {
        this.id = id;
    ...
    public static void init() {
        ...
        try{
            final File f = new File("./data/npcdump.bin");
            ...
            while(true){
                try{
                    if(buf.hasRemaining()){
                        ...
                        final String name = IoBufferUtils.getRS2String(buf);
                        int combat = buf.getUnsignedShort();
                        int hp = buf.getUnsignedShort();
                        final int spawnTime = buf.getUnsignedShort();
                        final int deathEmote = buf.getUnsignedShort();
                        final int blockEmote = buf.getUnsignedShort();
                        final int atkLenght = buf.getUnsigned();
                        ...
                        final int size = buf.getUnsigned();
                        ...
                        final NPCDefinition npcDef = new NPCDefinition(id, hp, combat, bonus, deathEmote, blockEmote, attacks, size, name, spawnTime);
                        ...
                        definition[8133] = NPCDefinition.create(8133, 1200, 650, bonus, 10059, 10053, new int[]{10057,
                                10058}, 4, "Corporeal_Beast", 184);
                        for(final int n : SummoningMonsters.SUMMONING_MONSTERS){
                            definition[n] = SummoningMonsters.loadDefinition(n);
                        }
                        for(final int n : RevAttack.getRevs()){
    ...
    public static final NPCDefinition create(final int id, final int maxHp, final int cb, final int[] bonus, final int deathAnim, final int blockAnim, final int[] atkAnims, final int size, final String name, final int spawnTime) {
    ...
    public int getAtkEmote(final int i) {
        return atkEmote[i];
    }
