...
public class FightPits implements ContentTemplate {
    ...
    public static final List<Integer> list = Arrays.asList(6585, 1712, 1731, 7462, 7461, 7460, 7459, 7458, 3024, 6685, 15330);
    ...
    private static final List<Integer> arrows = Arrays.asList(11212, 892, 890, 888);
    ...
    private final CopyOnWriteArrayList<NPC> monsters;
    ...
    public static final boolean inPitsFightArea(final int x, final int y) {
        return x <= 2420 && y >= 5129 && y <= 5168 && x >= 2375;
    }

    private static <V> V random(final List<V> list) {
    ...
    public static boolean canJoin(final Player player) {
        if(player.hardMode())
            return false;
        return !(ContentEntity.getTotalAmountOfEquipmentItems(player) > 0 || ContentEntity.getTotalAmountOfItems(player) > 0);
    }

    public static int getLifePointBoost(final Player player) {
        int boost = 0;
        final Container equip = player.getEquipment();
        for(final int i : rewardItems){
            final int slot = Equipment.getType(Item.create(i)).getSlot();
    ...
    public static int getBoost(final int slot) {
        switch(slot){
    ...
    public static void getClass(final Player player) {
        final int r = player.getExtraData().getInt("pitdeaths");
        switch(r){
            case 0:
    ...
    public static Object[] hasItems(final Player player) {
    ...
    public static boolean isSameTeam(final Player player, final Player opp) {
        return (teamBlue.contains(player) && teamBlue.contains(opp)) || (teamRed.contains(player) && teamRed.contains(opp));
    }

    public static boolean isBow(final int id) {
        for(final Item i : FightPits.rangeItems.get(WEAPON)){
    ...
    public static void fightPitsCheck(final Player player) {
        if(!FightPits.inPitsFightArea(player.getLocation().getX(), player.getLocation().getY()) && inGame(player) && !waitingRoom.contains(player) && !player.joiningPits){
    ...
    public static void dissapate(final Player player) throws StackOverflowError {
        Object[] data = hasItems(player);
        if(data != null){
            final int slot = (Integer) data[0];
            final boolean equip = (Boolean) data[1];
            if(equip)
    ...
    public static boolean isNoted(final ItemDefinition itemDef) {
        final ItemDefinition previous = ItemDefinition.forId(itemDef.getId() - 1);
        return previous != null && previous.getName().equalsIgnoreCase(itemDef.getName());
    }

    private static void normalize(final Player player) {
    ...
    public static void spawnItems(final Player player) {
        ...
        switch(player.pickedClass){
            case MAGE:
                for(int i = 0; i < 4; i++){
                    final List<Item> $list = mageItems.get(i);
            ...
            case MELEE:
                for(int i = 0; i < 4; i++){
                    final List<Item> $list = meleeItems.get(i);
                    player.getInventory().add($list.get(Misc.random($list.size() - 1)));
                }
                break;
            case RANGE:
                for(int i = 0; i < 4; i++){
                    final List<Item> $list = rangeItems.get(i);
                    player.getInventory().add($list.get(Misc.random($list.size() - 1)));
                }
    ...
    public static void spawnSpells(final Player player) {
        final Spells s = Spells.values()[Misc.random(Spells.values().length - 1)];
        if(!s.isMagik()){
        ...
        for(final int i : s.getRunes())
            player.getInventory().add(Item.create(i, 1000));

    }

    public static boolean inPits(final Player player) {
        return waitingRoom.contains(player) || playersInGame.contains(player);
    }

    public static String getMinutes(final int seconds) {
        final int minutes = seconds / 60;
        final int secondsRemaining = seconds % 60;
        final StringBuilder builder = new StringBuilder().append(minutes).append(":");
    ...
    private static void assignBlue(final Player player) {
    ...
    private static void assignRed(final Player player) {
    ...
    public static String getFoesRemaining(final Player player) {
        final StringBuilder builder = new StringBuilder("Foes Remaining: ");
    ...
    public static void getTeams() {
        ...
        for(final Player player : waitingRoom)
            player.getActionSender().sendString(2806, getFoesRemaining(player));
    }

    public static void removePlayerFromGame(final Player player, final boolean flag) {
        ...
        if(playersInGame.contains(player)){
            ...
            for(final Player player2 : playersInGame){
        ...
        if((teamRed.size() == 0 || teamBlue.size() == 0) && playersInGame.size() >= 1){
            ...
            final int size = winner.size();
            if(winner.size() == 0)
                return;
            if(EVENT)
                PushMessage.pushGlobalMessage(String.format("%s has just won fight pits with only %d player " + (size > 0 ? "s" : "") + " left!", winningTeam, size));
            for(final Player player2 : loser){
                player2.getAchievementTracker().fightPitsLose();
            }
            for(final Player player1 : winner){
                if(player1 == null)
                    continue;
                ...
                if(startPlayersAmount > 5 && getReward){
                    final int dpAmt = startPlayersAmount / size;
                    ...
                    if(Misc.random(60 * size) == 1){ //if 10 chance is 1/80 in total (half players each have chance of 1/400)
                        final Item reward = Item.create(random(rewardItems));
    ...
    public static boolean inGame(final Player player) {
    ...
    public void init() throws FileNotFoundException {
        ...
        for(int id = 14000; id < 15000; id++){
            final ItemDefinition itemDef = ItemDefinition.forId(id);
            String name = null;
            if(itemDef != null)
            ...
            if(name.contains("sacred") && name.contains("clay")){
                if(name.contains("needle") || name.contains("pouch") || name.contains("null") || name.contains("axe") || name.contains("pickaxe") || name.contains("harpoon") ||
            ...
            for(final List<Item> $list : meleeItems){
                for(final Item _item : $list){
                    if(_item != null)
                        scItems.add(_item.getId());
                }
            }
            for(final List<Item> $list : rangeItems){
                for(final Item _item : $list){
                    if(_item != null)
                        scItems.add(_item.getId());
                }
            }
            for(final List<Item> $list : mageItems){
                for(final Item _item : $list){
                    if(_item != null)
                        scItems.add(_item.getId());
                }
    ...
    public int[] getValues(final int i) {
        if(i == 6){
            final int[] responseIds = {9369, 9368, 30000, 31000, 30001};
            return responseIds;
        }else{
            if(i != 16)
                ;
    ...
    public boolean clickObject(final Player player, final int clickType, final int clickId, final int k, final int l, final int i1) {
    ...
    private void process() {
        if(playersInGame.size() > 1){
            if(--gameTimeLeft <= 0){
                for(final Player player : playersInGame){
                    if(player != null){
        ...
        final StringBuilder timeRemaining = new StringBuilder().append("Time left: ").append(getMinutes(timeLeft));
        if(EVENT)
    ...
    private void spawnNpc(final int i, final Location location) {
        final NPC npc = World.getWorld().getNPCManager().addNPC(location.getX(), location.getY(), 0, i, -1);
        npc.agressiveDis = 150;
        monsters.add(npc);
    }

    private void firstDoor(final Player player) {
        if(waitingRoom.contains(player)){
    ...
    private void secondDoor(final Player player) {
        if(playersInGame.contains(player)){
            player.getWalkingQueue().reset();
    ...
    public enum Spells {
        ...
        Spells(final boolean ancients, final int... runes) {
