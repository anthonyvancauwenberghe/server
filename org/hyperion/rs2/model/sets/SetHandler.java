...
public class SetHandler {
    public static final boolean handleSet(final Player player, final int id) {
        try{
                ...
                case 31421:
                    ...
                    if(ContentEntity.getTotalAmountOfEquipmentItems(player) == 0){
                        if(player.getSkills().getLevels()[Skills.DEFENCE] >= 70 && player.getSkills().getLevels()[Skills.ATTACK] >= 70){
                            SetUtility.getInstantSet(player, SetData.getMeleeSet());
                            SetUtility.addSetOfItems(player, SetData.getMeleeItems());
                            return true;
                        }else{
                ...
                case 31422:
                    ...
                    if(ContentEntity.getTotalAmountOfEquipmentItems(player) == 0){
                        if(player.getSkills().getLevels()[Skills.DEFENCE] >= 70 && player.getSkills().getLevels()[Skills.RANGED] >= 70){
                            SetUtility.getInstantSet(player, SetData.getRangeSet());
                            SetUtility.addSetOfItems(player, SetData.getRangeItems());
                            return true;
                        }else{
