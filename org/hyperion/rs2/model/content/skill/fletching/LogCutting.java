...
public class LogCutting extends Fletching {
    ...
    public static Log getLog(final int i) {
    ...
    private static cutItems getCutItem(final int i) {
        for(final cutItems item : cutItems.values()){
            if(item.getItemId() == i)
                return item;
        }
        return null;
    ...
    public static boolean chooseItem(final Player client, final int logId) {
        if(client.isBusy()){
            return true;
        }
        ...
        final Log log = getLog(logId);

        final cutItems[] items = log.getItems();
        if(items.length > 1){
    ...
    public static boolean startFletching(final Player client, final int amount, final int clicked) {
        if(client.isBusy()){
            return true;
        }
        client.getActionSender().removeAllInterfaces();
        client.setBusy(true);
        final Log log = getLog(client.getExtraData().getInt("logId"));

        if(log == null)
            return false;

        final cutItems item = log.getItems()[clicked];
        if(item == null)
            return false;
    ...
    public enum Log {
        ...
        private final cutItems[] items;

        Log(final int logId, final cutItems... items) {
            this.logId = logId;
            this.items = items;
        }
    ...
    public enum cutItems {
        ...
        cutItems(final int itemId, final int levelReq, final int exp) {
            this.itemId = itemId;
