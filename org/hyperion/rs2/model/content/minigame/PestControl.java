...
public class PestControl implements ContentTemplate {
 ...
 Torcher 	33, 49, 79, 91, 92 	Mage		Yes		same as defiler				
    ...
    public void spawnNpc(final int i, final Location location, final Player player) {
    ...
    public void process() {
        if(playersInGame.size() <= 0){
            if(waitTimer > 0)
                ...
                //tele all to arena
                startUpGame();
                for(final Player player : waitingBoat){
            ...
            @SuppressWarnings("unused") boolean win = false;
            if(gameTimer > 0)
                gameTimer--;
    ...
    public void killNpcs() {
        for(final PestNPC pnpc : monsters){
            killNPC(pnpc);
        }
        if(voidKnight != null)
            killNPC(voidKnight);
    ...
    public void killNPC(final PestNPC pnpc) {
        pnpc.npc.health = 0;
    ...
    public static class PestNPC {
        ...
        public PestNPC(final NPC npc, final int type) {
            this.npc = npc;
            this.type = type;
        }
    }
