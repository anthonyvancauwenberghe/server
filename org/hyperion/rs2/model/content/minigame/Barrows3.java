...
import org.hyperion.rs2.model.ExtraData;
...
public class Barrows3 implements ContentTemplate {
    ...
    public static final int[] BONUS_REWARDS = {4740, 4694, 4695, 4696, 4697, 4698, 4699,};
    ...
    public static int boolArrayToInt(final boolean[] array) {
        int total = 0;
    ...
    public static boolean[] intToBoolArray(int integer, final int size) {
        final boolean[] array = new boolean[size];
        for(int i = size - 1; i >= 0; i--){
            final int idxvalue = (int) Math.pow(2, i);
            if(integer >= idxvalue){
                array[i] = true;
                integer -= idxvalue;
            }
    ...
    public static int npcForCoffin(final int id) {
        switch(id){
            case 6771:
    ...
    public static int newItemId(final int item) {
    ...
    public static void confirmCoffinTeleport(final Player player) {
        World.getWorld().submit(new Event(100) {
            ...
            public void execute() {
                ...
                final NPC n = World.getWorld().getNPCManager().addNPC(3553, 9694, 0, player.getExtraData().getInt(BROTHER_TARGET), -1);
    ...
    public static boolean clickCoffin(final Player client, final int oId, final int oX, final int oY) {
        ...
        if(client.getExtraData().getInt(BROTHER_TARGET) == brotherId){ //if target brother
            ...
            final boolean[] brothersKilled = intToBoolArray(client.getExtraData().getInt(BROTHERS_KILLED_KEY), 6);
            ...
            if(!client.hasTarget()){
                //spawn npc
                final NPC n = World.getWorld().getNPCManager().addNPC(client.getLocation().getX(), client.getLocation().getY(), client.getLocation().getZ(), npcForCoffin(oId), -1);
                n.forceMessage("You dare disturb my slumber!");
                n.agressiveDis = 7;
    ...
    public void openChest(final Player player) {
        if(player.getExtraData().getInt(KILLSCOUNT_KEY) >= 3){
            final int rewardscount = player.getExtraData().getInt(KILLSCOUNT_KEY);
            for(int i = 0; i < rewardscount; i++){
                final Item barrowsReward = new Item(REWARDS[Misc.random(REWARDS.length - 1)]);
            ...
            if(player.getInventory().freeSlots() > 0){
                final Item bonusReward = new Item(BONUS_REWARDS[Misc.random(BONUS_REWARDS.length - 1)]);
    ...
    public void killNpc(final Player client, final int id) {
        System.out.println("Id killed: " + id);
        int killsInt = client.getExtraData().getInt(BROTHERS_KILLED_KEY);
        final boolean[] killedbrothers = intToBoolArray(killsInt, 6);
    ...
    public int[] getValues(final int type) {
        ...
        if(type == 6){
            final int[] j = {6771, 6772, 6773, 6821, 6822, 6823,/*stairs*/6707, 6706, 6705, 6702, 6704, 6703,/*chest*/
                    10284};
            return j;
        }
        if(type == 16){
            final int[] j = {2025, 2026, 2027, 2028, 2029, 2030,};
            return j;
        }
        return null;
    }
