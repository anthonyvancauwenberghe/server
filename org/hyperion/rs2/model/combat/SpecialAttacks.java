...
public class SpecialAttacks {
    ...
    public static boolean clickedSpecialButton(final Player player) {
        if(player.getEquipment().get(3) == null)
            return false;
        final int weaponId = player.getEquipment().get(3).getId();
        switch(weaponId){
            case 4153:
                // maul
                if(player.cE.getOpponent() != null){
                    final String message = Combat.canAtk(player.cE, player.cE.getOpponent());
                    if(message.length() > 1){
                        return false;
                    }
                    player.specOn = false;
                    // int maxHit = 0;
                    final int maxHit = CombatAssistant.calculateMaxHit(player);
    ...
    public static int defence(final CombatEntity combatEntity, final int maxDamg, final int atkBonus) {
        if(combatEntity.getOpponent().getEntity() instanceof Player){
            if(Combat.random(CombatAssistant.calculateMeleeAttack(combatEntity.getPlayer()) + atkBonus) < Combat.random(CombatAssistant.calculateMeleeDefence(combatEntity.getOpponent().getPlayer())))
                return 0;
        }else{
            if(Combat.random(CombatAssistant.calculateMeleeAttack(combatEntity.getPlayer()) + atkBonus) < Combat.random(combatEntity.getOpponent().getNPC().getDefinition().getBonus()[(combatEntity.getAtkType() + 2)]))
                return 0;
        }
        return maxDamg;
    }
    ...
    public static boolean special(final Player player, int maxDamg, final int weaponId, final int currentdistance, final int combatStyle) {
        final CombatEntity combatEntity = player.getCombat();
        ...
        switch(weaponId){
            ...
            case 15703:
            case 15704:
                final int bowType = CombatAssistant.getCombatStyle(player.cE);
                if(bowType <= Constants.NOAMMO){
                    player.getActionSender().sendMessage("You have no arrows left in your quiver.");
                }
                playerGfx = -1;
                specialAnimation = 426;
                specialDis = 9;
                specialDrain = 60;
                minimum = 5;
                if(player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212){
		 ...
		 */
        if(!player.getLocation().isWithinDistance(player.cE.getOpponent().getEntity().getLocation(), (specialDis + (player.cE.canMove() ? 1 : 0)))){
            player.specOn = true;
            if(player.cE.getOpponent().getEntity() instanceof Player)
                player.getActionSender().follow(player.cE.getOpponent().getEntity().getIndex(), 1);
        ...
        if(player.getSpecBar().getAmount() < specialDrain){
            player.getActionSender().sendMessage("You dont have enough special energy to do this attack.");
        ...
        if(player.cE.getOpponent().getEntity() instanceof Player){
            if(!ranged)
                deltaBonus = (int) (CombatAssistant.calculateMeleeAttack(player) * specialAccuracy) - CombatAssistant.calculateMeleeDefence(player.cE.getOpponent().getPlayer());
            else
                deltaBonus = (int) (CombatAssistant.calculateRangeAttack(player) * specialAccuracy) - CombatAssistant.calculateRangeDefence(player.cE.getOpponent().getPlayer());

        }else{
            if(!ranged)
                deltaBonus = (int) (CombatAssistant.calculateMeleeAttack(player) * specialAccuracy) - (int) ((player.cE.getOpponent().getCombat() / 1.6) + 64);
            else
                deltaBonus = (int) (CombatAssistant.calculateRangeAttack(player) * specialAccuracy) - (int) (player.cE.getOpponent().getCombat() / 1.5);
        }
        ...
        final int critical = hitDamage > maxDamg * 0.9 ? 5 : 0;
        /*
        ...
        if(player.cE.getWeaponPoison() > 0 && Combat.random(10) <= player.cE.getWeaponPoison() && !player.cE.getOpponent().isPoisoned())
            Combat.poisonEntity(player.cE.getOpponent());

        if(player.cE.getOpponent().getCurrentAtker() == null || player.cE.getOpponent().getCurrentAtker() == player.cE){
            player.cE.getOpponent().face(player.cE.getAbsX(), player.cE.getAbsY());
            if(player.cE.getOpponent().getEntity() instanceof Player || player.cE.getOpponent().getNPC().getDefinition().doesDefEmote())
                player.cE.getOpponent().doDefEmote();
            if(player.cE.getOpponent().getEntity() instanceof NPC || player.cE.getOpponent().getPlayer().autoRetailate){
            ...
            if(delayedWeapon(weaponId)){
                World.getWorld().submit(new Event(delay, "combat") {
                    public void execute() {
                        ...
                        if(oldEntity.getEntity() instanceof Player){
                            Magic.vengeance(oldEntity.getPlayer(), player.cE, hitDamage);
                        }
                        this.stop();
                    }
                });
            }else{
                player.cE.getOpponent().hit(hitDamage, player, false, cbStyle + crit);
                if(oldEntity.getEntity() instanceof Player)
                    Magic.vengeance(oldEntity.getPlayer(), player.cE, hitDamage);
            }
        ...
        switch(weaponId){
            ...
            case 15704:
                ...
                if(player.getCombat().getOpponent() != null && player.getCombat().getOpponent().getEntity() instanceof Player){
                    deltaBonus2 = CombatAssistant.calculateRangeAttack(player) - CombatAssistant.calculateRangeDefence(player.getCombat().getOpponent().getPlayer());
                    deltaBonus3 = CombatAssistant.calculateRangeAttack(player) - CombatAssistant.calculateRangeDefence(player.getCombat().getOpponent().getPlayer());
                ...
                int offsetY = ((player.cE.getAbsX()) - player.cE.getOpponent().getAbsX()) * -1;
                int offsetX = ((player.cE.getAbsY()) - player.cE.getOpponent().getAbsY()) * -1;
                ...
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed - 10, player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099 : 1102, 46, 31, hitId, 3);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed + 10, player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099 : 1102, 46, 36, hitId, slope + 6);
                    ...
                    World.getWorld().submit(new Event(200 * distance + 300, "combat") {
                        public void execute() {
                            ...
                            if(entity.getEntity() instanceof Player)
                                Magic.vengeance(oldEntity.getPlayer(), player.cE, hit1);
                            this.stop();
                        }
                    });

                    entity.getEntity().playGraphics(Graphic.create(player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1100 : 1103, 6553600 + clientSpeed));
                }
                break;
            ...
            case 13902:
                ...
                if(player.getCombat().getOpponent() != null){
                    player.getCombat().getOpponent()._getPlayer().ifPresent(p -> {
                        final int lvl = p.getSkills().getLevel(1);
                        final int remove = weaponId == 13902 ? hitDamage / 2 : hitDamage;
                        final int toRemove = lvl - remove < 1 ? lvl - 1 : remove;
            ...
            case 19780:
                ...
                if(dmg < maxDamg / 2)
                    dmg = maxDamg / 2;
                if(player.getPrayers().isEnabled(48))
                    Prayer.soulSplit(player, player.cE.getOpponent(), dmg);
                if(player.cE.getOpponent().getEntity() instanceof Player){
                    Magic.vengeance(player.cE.getOpponent().getPlayer(), player.cE, dmg);
            ...
            case 5680:
                ...
                deltaBonus = (int) (CombatAssistant.calculateMeleeAttack(player) * specialAccuracy) - CombatAssistant.calculateMeleeDefence(oldEntity.getEntity());
                ...
                */
                ...
                World.getWorld().submit(new Event(delay, "combat") {
                    ...
                    public void execute() throws IOException {
                        oldEntity.hit(damg5, player, false, crit);
                ...
                break;
            case 11730:
                final int damage = Misc.random(16);
                player.cE.getOpponent().hit(damage, player, false, Constants.MAGE);
            ...
            case 17640:
                ...
                try{
                    if(player.cE.getOpponent().getEntity() instanceof Player){
                        final Player opponent = player.cE.getOpponent().getPlayer();
            ...
            case 14484:
                ...
                if(hitDamage > 0){
                    player.cE.getOpponent().hit((int) hitDamage / 2, player, false, 0);
                    World.getWorld().submit(new Event(1000) {
                        @Override
                        public void execute() {
                            final String message = Combat.canAtk(player.cE, oldEntity);
                            if(message.length() > 1){
                                this.stop();
                                return;
                            }

                            oldEntity.hit((int) (hitDamage / 4), player, false, 0);
                            oldEntity.hit((int) (hitDamage / 4) + 1, player, false, 0);
                            this.stop();
                        }
                    });
                }else{
                    ...
                    oldEntity.hit(maxDamg3, player, false, 0);
                    ...
                    World.getWorld().submit(new Event(1000) {
                        ...
                        public void execute() {
                            ...
                            if(maxDamg3 <= 0){
                                ...
                                oldEntity.hit((int) maxDamg4, player, false, 0);
                                ...
                                if(maxDamg4 == 0)
                                    oldEntity.hit(maxDamg5, player, false, 0);
                                else
                                    oldEntity.hit((int) (maxDamg4 / 2), player, false, 0);
                            }else{
                                oldEntity.hit((int) (maxDamg3 / 2), player, false, 0);
                                oldEntity.hit((int) (maxDamg3 / 2), player, false, 0);
                            }
                            this.stop();
                        }
            ...
            case 4587:
                if(hitDamage > 0 && player != null && player.cE != null && player.cE.getOpponent() != null){
                    if(player.cE.getOpponent().getEntity() instanceof Player){
                        final Player prayerOff = player.cE.getOpponent().getPlayer();
                        if(prayerOff != null){
            ...
            case 859:
                ...
                offsetY = ((player.cE.getAbsX()) - player.cE.getOpponent().getAbsX()) * -1;
                offsetX = ((player.cE.getAbsY()) - player.cE.getOpponent().getAbsY()) * -1;
                ...
                // 11212 ? 1099 : 1101, showDelay, 50, clientSpeed + 10, 46, 36,
                // victim.getProjectileLockonIndex(), slope + 6, 36));
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed - 10, 1099, 46, 45, hitId, 3);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed + 10, 256, 46, 40, hitId, 6);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed - 10, 1099, 46, 35, hitId, 9);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed + 10, 256, 46, 30, hitId, 12);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed - 10, 1099, 46, 25, hitId, 15);
                    ...
                    World.getWorld().submit(new Event(200 * distance + 300, "combat") {
                        public void execute() {
                            ...
                            if(entitytt.getEntity() instanceof Player)
                                Magic.vengeance(oldEntity.getPlayer(), player.cE, hit1tt);
                            this.stop();
                        }
                    });

                    entitytt.getEntity().playGraphics(Graphic.create(player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1100 : 1103, 6553600 + clientSpeed));
                }
                break;
            case 861:
                // TODO
                ...
                // 11212 ? 1099 : 1101, showDelay, 50, clientSpeed + 10, 46, 36,
                // victim.getProjectileLockonIndex(), slope + 6, 36));
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed - 10, 256, 46, 31, hitId, 3);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed + 10, 256, 46, 36, hitId, slope + 6);
                player.getActionSender().createGlobalProjectile(player.cE.getAbsY(), player.cE.getAbsX(), offsetY, offsetX, 50, clientSpeed, 256, 46, 41, hitId, slope + 9);
                damg4 = Combat.random(maxDamg);
                damg8 = Combat.random(maxDamg);
                damg4 = damg4 + toAddFirst;
                damg8 = damg8 + toAddSecond;
                    ...
                    World.getWorld().submit(new Event(200 * distance + 300, "combat") {
                        public void execute() {
                            ...
                            if(entityt.getEntity() instanceof Player)
                                Magic.vengeance(oldEntity.getPlayer(), player.cE, hit1t);
                            this.stop();
                        }
                    });

                    entityt.getEntity().playGraphics(Graphic.create(player.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1100 : 1103, 6553600 + clientSpeed));
                }
                break;
        }
    ...
    private static boolean delayedWeapon(final int weaponId) {
        switch(weaponId){
            case 14484:
            case 4153:
            case 17646:
     ...
     */
    public static double getSpecialBonus(final Player p) {
        double specDamage = 1;
        final Item weapon = p.getEquipment().get(Equipment.SLOT_WEAPON);
        if(weapon == null)
            return specDamage;
        switch(weapon.getId()){
            case 17640:
