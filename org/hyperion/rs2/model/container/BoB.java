...
public class BoB {
    ...
    public static void openInventory(final Player player) {
        ...
        player.getInterfaceState().addListener(player.getBoB(), new InterfaceContainerListener(player, BOB_INVENTORY_INTERFACE));
        player.getInterfaceState().addListener(player.getInventory(), new InterfaceContainerListener(player, PLAYER_INVENTORY_INTERFACE - 1));

    }

    public static void dropBoB(final Location loc, final Player player) {
        if(player == null || player.getBoB() == null || player.isHidden())
            return;
    ...
    public static void withdraw(final Player player, final int slot, final int id, final int amount) {
        ...
        if(slot < 0 || slot > player.getBoB().capacity() || id < 0 || id > ItemDefinition.MAX_ID)
            return;
        final Item item = player.getBoB().get(slot);
        if(item == null){
        ...
        if(def.isStackable()){
            if(player.getInventory().freeSlots() <= 0 && player.getInventory().getById(newId) == null){
                player.getActionSender().sendMessage("You don't have enough inventory space to withdraw that many."); // this
                // is
                // the
                // real
                // message
    ...
    public static void deposit(final Player player, final int slot, final int id, final int amount, final Container container) {
        if(!Server.SPAWN){
            return;
        }
        if(player.cE.summonedNpc == null){
        ...
        final boolean inventoryFiringEvents = container.isFiringEvents();
        ...
        try{
            ...
            if(item.getDefinition().isStackable() || noted){
                ...
                if(player.getBoB().freeSlots() < 1 && player.getBoB().getById(bankedId) == null){
                    player.getActionSender().sendMessage("You don't have enough space in your summon's pouch."); // this
                    // is
                    // the
                    // real
                    // message
                ...
                if(!player.getBoB().add(new Item(item.getId(), transferAmount))){
                    player.getActionSender().sendMessage("You don't have enough space in your summon's pouch."); // this
                    // is
                    // the
                    // real
