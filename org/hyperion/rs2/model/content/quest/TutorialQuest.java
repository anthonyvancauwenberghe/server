...
public class TutorialQuest implements ContentTemplate {
    ...
    public static void walkDialogue(final Player player) {
    ...
    public void talkToGuide(final Player player, final int npcSlot) {
    ...
    public boolean clickObject(final Player player, final int type, final int a, final int b, final int c, final int d) {
        ...
        if(type == 10){
            ...
            if(++player.tutSubIsland >= 3){
                ...
                final GlobalItem globalItem = new GlobalItem(player, b, c, 0, new Item(2138, 1));
                World.getWorld().getGlobalItemManager().newDropItem(player, globalItem);
            }
        }else if(type == 6){
            if(a == 1738){
                //player.setTeleportTarget(Location.create(2897, 3513, 1));
                final GlobalItem globalItem = new GlobalItem(player, 2897, 3507, 1, new Item(1856, 1));
            ...
            player.getActionSender().openQuestInterface("DeviousPK Guidebook", new String[]{
                    "Welcome newcomer to ArteroPK,", "created by Arre ",
                    "Please be updated with all the latest news on the website", " https://arteropk.com",
                    "Your bank has been loaded with a starter pack!", "", "-Master combat stats ::master",
                    ...
                    "-Skill through skill teleports in magic spells", "-Tormented Demons can be found above the KBD",
	...
	*/
    ...
    public int[] getValues(final int type) {
        ...
        if(type == 6){
            final int[] j = {1738, 1740};//stairs
            return j;
        }
        if(type == 1){
            final int[] j = {1856};//book
            return j;
        }
        // TODO Auto-generated method stub
        return null;
