...
public class Fletching implements ContentTemplate {
    ...
    private final int[] fletchingTools = {314, 53};
    ...
    public boolean isFletchable(final Player client, final int slot1, final int slot2, final int useItem, final int usedItem) {
        ...
        //BowStringing
            ...
            final BowStringing.StrungItems[] items = BowStringing.getString(usedItem).getItems();
            ...
            final BowStringing.StrungItems[] items = BowStringing.getString(useItem).getItems();
            for(int i = 0; i < items.length; i++){
        ...
        //ArrowMaking
            ...
            final ArrowMaking.Arrow item = ArrowMaking.getArrow(usedItem);
            if(item != null)
                ArrowMaking.createArrows(client, usedItem);
        }else if(usedItem == 53){
            final ArrowMaking.Arrow item = ArrowMaking.getArrow(useItem);
            if(item != null)
                ArrowMaking.createArrows(client, useItem);
        }
        //DartMaking
        else if(useItem == 314){
            final DartMaking.Dart item = DartMaking.getDart(usedItem);
            if(item != null)
                DartMaking.createDarts(client, usedItem);
        }else if(usedItem == 314){
            final DartMaking.Dart item = DartMaking.getDart(useItem);
            if(item != null)
    ...
    public int[] getValues(final int type) {
        if(type == 13){
            ...
            for(final ArrowMaking.Arrow arrow : ArrowMaking.Arrow.values()){
                j.add(arrow.getArrowHeadId());
            }
            for(final BowStringing.StringItem string : BowStringing.StringItem.values()){
                j.add(string.getItemId());
            }
            for(final BowStringing.StrungItems item : BowStringing.StrungItems.values()){
                j.add(item.getItemId());
            }
            for(final DartMaking.Dart dart : DartMaking.Dart.values()){
