...
public class LeatherCrafting extends Crafting {
    ...
    private static Leather_Item getLeatherItem(final int i) {
        for(final Leather_Item item : Leather_Item.values()){
            if(item.getItemId() == i)
                return item;
        }
        return null;
    ...
    private static Leather getLeather(final int i) {
        for(final Leather l : Leather.values()){
            if(l.getItemId() == i)
                return l;
        }
        return null;
    ...
    public static boolean craftLeather(final Player c, final int item) {
        try{
            if(c.isBusy())
                return true;
            final Leather l = getLeather(item);
            if(l == null){
                return true;
            }
            ...
            c.setBusy(true);
            final Leather_Item[] items = l.getItems();
            if(items.length > 1){
    ...
    public static boolean startAgain(final Player c, final int amm, final int slot) {
        final Leather leather = getLeather(c.getExtraData().getInt("craftingFrom"));

        if(leather == null)
            return false;

        final Leather_Item item = leather.getItems()[slot];
        if(item != null)
    ...
    public static void finishCraft(final Player c, final int amm) {
        ...
        final Leather_Item item = getLeatherItem(c.getExtraData().getInt("toCraft"));
        final Leather leather = getLeather(c.getExtraData().getInt("craftingFrom"));
    ...
    public enum Leather {
        ...
        private final Leather_Item[] items;

        Leather(final int itemId, final Leather_Item... items) {
            this.itemId = itemId;
            this.items = items;
        }
    ...
    public enum Leather_Item {
        ...
        private final int amountReq;
        private final int exp;

        Leather_Item(final int itemId, final int amountReq, final int levelReq, final int exp) {
            this.itemId = itemId;
