...
public class Duel {
    ...
    public static final int DUEL_RULE_ID[] = {1, 2, 16, 32, 64, 128, 256, 512, 1024, 4096, 8192, 16384, 32768, 0x10000,
            0x20000, 0x40000, 0x80000, 0x200000, 0x800000, 0x1000000, 0x4000000, 0x8000000};
    public static String rulesOption[] = new String[9];

    public Duel() {
    }

    public static void open(final Player player, final Player opponent) {
    ...
    public static void withdraw(final Player player, final int slot, final int id, final int amount) {
        ...
        final int newId = item.getId(); // TODO deal with withdraw as notes!
    ...
    public static String listConfirmScreen(final Item[] items) {
        ...
        for(final Item item : items){
            ...
            if(item.getId() > 0){
                if((item.getCount() >= 1000) && (item.getCount() < 1000000)){
                    sendAmount = "@cya@" + (item.getCount() / 1000) + "K @whi@(" + NameUtils.formatInt(item.getCount()) + ")";
                }else if(item.getCount() >= 1000000){
                    sendAmount = "@gre@" + (item.getCount() / 1000000) + " million @whi@(" + NameUtils.formatInt(item.getCount()) + ")";
                }else{
    ...
    public static void confirmScreen(final Player player) {
        if(player.duelAttackable > 0)
            return;
        ...
        final String[] as = {"Players cannot forfeit!", "Players cannot move.", "Players cannot use range.",
        ...
        final String s = listConfirmScreen(player.getDuel().toArray());
        final String s1 = listConfirmScreen(player.getTrader().getDuel().toArray());
        ...
        player.getTrader().getActionSender().sendUpdateItems(3214, player.getTrader().
                getInventory().toArray());
        player.onConfirmScreen = true;
        player.getTrader().onConfirmScreen = true;
    }

    public static int getItemsRemovedCount(final Player player) {
        int i = 0;
        for(int j = 0; j < 14; j++){
    ...
    public static void finishTrade(final Player player) {
        if(player.getTrader() == null)
            return;
        if(World.getWorld().updateInProgress()){
            player.getActionSender().sendMessage("You can't duel during an update.");
    ...
    public static boolean inDuelLocation(final Player player) {
        return player != null && inDuelLocation(player.getLocation().getX(), player.getLocation().getY());
    }

    public static boolean inDuelLocation(final int x, final int y) {
    ...
    public static void declineTrade(final Player player) {
        player.onConfirmScreen = false;
        //player.cannotSwitch = false;
    ...
    public static void startDueling(final Player player) {
        ...
        if(player.duelAttackable > 0)
            return;
        player.getLogManager().add(LogEntry.duel(player, player.getTrader()));
        player.getTrader().getLogManager().add(LogEntry.duel(player, player.getTrader()));
    ...
    public static void removeBanEquip(final Player player) {
    ...
    public static void finishDuel(final Player player, final Player opponent, final boolean won) {
    ...
    public static void finishFullyDuel(final Player player) {
        try{
            final Player player1 = player.getTrader();
            if(player1 != null){
    ...
    public static void selectRule(final Player player, final int i, final boolean flag, final int j) {
        ...
        if(!player.duelRule[i]){
            ...
            if(rule != null)
                ...
                player.duelRuleOption |= DUEL_RULE_ID[i];
        }else{
            player.duelRule[i] = false;
            if(rule != null)
                player.duelRuleOption &= ~DuelRule.forId(i).getFlag();
    ...
    public static void teleportToArena(final Player player) {
        final int x1 = 3336 + Combat.random(12);
