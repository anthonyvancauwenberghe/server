...
public class AccountValuesRequest extends SQLRequest {
    ...
    public void process(final SQLConnection sql) {
        ...
        final String query = String.format("INSERT INTO accountvalues (name, value, pkvalue,ip,mac,suid) VALUES ('%s', %d, %d, '%s', %d, '%s') ON DUPLICATE KEY UPDATE value = %d, pkvalue = %d, ip='%s', mac=%d, suid='%s'", player.getName().toLowerCase(), value, pkpValue, ip, mac, specialUid, value, pkpValue, ip, mac, specialUid);
