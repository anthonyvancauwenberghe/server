...
public class Dungeon {
    ...
    public final void remove(final Player player, final boolean complete) {
        ...
        if(complete){
            ...
            final long delta_time = (long) (difficulty.time * size.multi_time) - elapsed_time;
            final long time = TimeUnit.MINUTES.convert(delta_time, TimeUnit.MILLISECONDS);
            double multiplier = (time / 10D) + 1.0;
            if(multiplier > 1.5)
                multiplier = 1.5;
            if(multiplier < 0.5)
                multiplier = 0.5;
            final int death = deaths.getOrDefault(player, 0);
            double death_penalty = Math.pow(0.85, death);
            if(death_penalty < 0.4)
                death_penalty = 0.4;
            final double team_penalty = Math.pow(1.04, teamSize - 1);
            final double size_multi = size.multiplier;
            final int xp = (int) ((difficulty.xp * multiplier) * death_penalty * size_multi * team_penalty);
            final int tokens = xp / 30;
            ...
            player.getAchievementTracker().dungFloorCompleted(DungeoneeringFloorsTask.Difficulty.valueOf(difficulty.name()), DungeoneeringFloorsTask.Size.valueOf(size.name()));
    ...
    public void kill(final Player player) {
        final int old = deaths.getOrDefault(player, 0);
