...
public class Dicing implements ContentTemplate {
    ...
    public static void setBadLuckDicer(final String name) {
        badLuckDicer = name;
    }

    private static void startRollingDice(final Player player) {
    ...
    public static synchronized void put(final int k, final int v) {
        gambled.put(k, v);
    }

    public static synchronized void remove(final int k) {
        gambled.remove(k);
    }

    public static synchronized Integer get(final int k) {
        return gambled.get(k);
    }

    public static synchronized Item[] getGambledItems() {
        final List<Item> item = new LinkedList<Item>();
        for(final int k : gambled.keySet()){
     ...
     * @param item
     */
    public static void diceNpc(final Player player, final NPC dicer, final Item item, final boolean toPkp) {
        ...
        World.getWorld().submit(new Event(2000, "checked") {
            ...
            public void execute() {
                ...
                final int itemvalue = DonatorShop.getPrice(id) * count;
                ...
                if(r >= 55){
                    ...
                    query = "INSERT INTO dicing(username,item_id,item_count,win_value) " + "VALUES('" + player.getName().toLowerCase() + "'," + id + "," + count + "," + itemvalue + ")";
                    ...
                    player.setDiced(player.getDiced() - itemvalue);
                    query = "INSERT INTO dicing(username,item_id,item_count,win_value) " + "VALUES('" + player.getName().toLowerCase() + "'," + id + "," + count + "," + -1 * itemvalue + ")";
                }
                if(itemvalue > 0){
    ...
    public static void rollPrivateDice(final Player player) {
        ...
        World.getWorld().submit(new Event(3000) {
            public void execute() {
                final int thrown = getRandomNumber(player, 100);
    ...
    public static int getRandomNumber(final Entity entity, final int n) {
    ...
    public void init() throws FileNotFoundException {
        try{
            final File dontoPkp = new File("./data/dontopkp.txt");
            if(!dontoPkp.exists()){
        ...
        try{
            final File diceClanFile = new File("./data/diceclans.txt");
    ...
    @Override
    public boolean dialogueAction(final Player player, final int id) {
