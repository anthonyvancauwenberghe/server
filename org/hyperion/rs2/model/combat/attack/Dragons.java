...
public class Dragons implements Attack {
    ...
    public int handleAttack(final NPC n, final CombatEntity attack) {
        ...
        if(distance < (7 + ((n.getDefinition().sizeX() + n.getDefinition().sizeY()) / 2))){
            ...
            if(attack.getPlayer() == null)
                return 1;
            final int attackId = Combat.random(20);
            /*
             *
            ...
            if(attackId > 6 && distance <= (1 + (n.getDefinition().sizeX() + n.getDefinition().sizeY()) / 2)){
                ...
                n.cE.predictedAtk = (System.currentTimeMillis() + 3000);
                final boolean antiFire = attack.getPlayer() != null && (System.currentTimeMillis() - attack.getPlayer().antiFireTimer < 360000) && attack.getPlayer().superAntiFire;
                if(antiFire)
    ...
    @Override
    public int[] npcIds() {
        final int[] j = {53, 54, 55, 941, 1592, 1591, 1590, 5363,};
        return j;
    }
