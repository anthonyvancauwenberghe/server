    ...
    public void handlePacket(final LoginPacket packet) {
        ...
        switch(packet.getOpcode()){
            case LoginPacket.CHECK_LOGIN:{
                ...
                final LoginResult res = server.getLoader().checkLogin(new PlayerDetails(null, name, password, 0, null, null, "", "Id2"));
                if(res.getReturnCode() == 2){
                    final PlayerData pd = new PlayerData(name, (int) Rank.getPrimaryRankIndex(res.getPlayer()));
            ...
            case LoginPacket.LOAD:{
                final String name = NameUtils.formatNameForProtocol(IoBufferUtils.getRS2String(buf));
                final Player p = new Player(new PlayerDetails(null, name, "", 0, null, null, "", "Id3"), false);
            ...
            case LoginPacket.SAVE:{
                ...
                dataBuffer.put(data);
                dataBuffer.flip();
                final Player p = new Player(new PlayerDetails(null, name, "", 0, null, null, "", "Id4"), false);
                //if(PlayerFiles.exists(name))
                PlayerFiles.saveGame(p);
                //else
                //p.deserialize(dataBuffer,false);
                System.out.println("NODE");
                final int code = server.getLoader().savePlayer(p, "Loginpacket save") ? 1 : 0;
                final IoBuffer resp = IoBuffer.allocate(16);
                resp.setAutoExpand(true);
                IoBufferUtils.putRS2String(resp, name);
                resp.put((byte) code);
