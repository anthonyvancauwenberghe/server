...
public final class LogUtils {
    ...
    public static String toString(final Item[] items) {
        try{
            return Stream.of(items).filter(Objects::nonNull).collect(Collectors.groupingBy(Item::getId, Collectors.summingInt(Item::getCount))).entrySet().stream().map(e -> Item.create(e.getKey(), e.getValue().intValue())).map(LogUtils::toString).collect(Collectors.joining(", "));
