...
public class Spam {
    ...
    static {
        CommandHandler.submit(new Command("teletospammer", Rank.MODERATOR) {
            ...
            public boolean execute(final Player player, final String input) {
                final LinkedList<Spam> spammers = new LinkedList<Spam>();
                for(final Player spammer : World.getWorld().getPlayers()){
                    if(spammer.getSpam().isSpamming()){
                        spammers.add(spammer.getSpam());
                    }
                }
                if(spammers.size() > 0){
                    final int randomIdx = Misc.random(spammers.size() - 1);
                    final Spam spam = spammers.get(randomIdx);
     ...
     */
    private final List<byte[]> lastMessages = new LinkedList<byte[]>();
    /**
     * The player.
     */
    private final Player spammer;
    /**
     ...
     * @param player
     */
    public Spam(final Player player) {
        this.spammer = player;
    }
     ...
     */
    public static void warnModerators(final Player spammer) {
        final String warning = "@dre@[Important] Possibly someone spamming with username: " + spammer.getName();
        final String extraInfo = "@dre@[Important] Location: " + spammer.getLocation();
     ...
     * @param lastMessage
     */
    public void checkSpam(final byte[] lastMessage) {
        ...
        if(System.currentTimeMillis() - spammer.getCreatedTime() > Time.ONE_WEEK)
            return;
        for(final byte[] message : lastMessages){
            if(Arrays.equals(message, lastMessage)){
                final int counter = spammer.getExtraData().getInt(COUNTER_KEY);
     ...
     */
    public void setHunting(final boolean b) {
        if(b)
