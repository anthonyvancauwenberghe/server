...
public class TestCommands {
    ...
    public static boolean processBetaCommands(final Player player, final String commandStart, final String s, final String withCaps, final String[] as) throws IOException {
        ...
        if(commandStart.equalsIgnoreCase("giveclues")){
            for(final ClueScroll clue : ClueScrollManager.getAll())
        ...
        if(commandStart.equalsIgnoreCase("resetvote")){
            try{
                ...
                final String yesterday = new SimpleDateFormat("dd/MM/yyyy").format(cal.getTime());
        ...
        if(commandStart.equalsIgnoreCase("item")){
            final String input = s.substring(5).trim();
