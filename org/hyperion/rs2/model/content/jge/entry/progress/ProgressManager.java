...
public class ProgressManager {
    ...
    public static ProgressManager fromSaveString(final Entry entry, final String progress) {
        ...
        if(progress.isEmpty())
            return manager;
        Stream.of(progress.split(",")).map(Progress::fromSaveString).forEach(manager::add);
        return manager;
    }
    ...
    public ProgressManager copy() {
        ...
        stream().map(Progress::copy).forEach(copy::add);
        return copy;
    }
    ...
    public int totalPrice() {
        switch(entry.type){
            case BUYING:
                return stream().mapToInt(p -> p.unitPrice).sum() * totalQuantity();
            case SELLING:
    ...
    public int totalQuantity() {
        return stream().mapToInt(p -> p.quantity).sum();
    }
    ...
    public String toSaveString() {
        return stream().map(Progress::toSaveString).collect(Collectors.joining(","));
    }
}
