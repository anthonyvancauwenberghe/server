...
public class Mining implements ContentTemplate {
    ...
    private static final int[] ROCKS = {2491, 2108, 2109, 2090, 2091, 2094, 2095, 2110, 4030, 2092, 2093, 2101, 3403,
            2096, 2097, 2099, 2102, 2103, 2104, 2105, 2107,/*doors into guild*/ 2112, 1755, 2113};
    private static final int[] ROCKS_ORES = {1436, 434, 434, 436, 436, 438, 438, 668, 3211, 440, 440, 442, 2892, 453,
            453, 444, 447, 447, 449, 449, 451};
    private static final int[] ROCK_REQUIRED_LEVELS = {1, 1, 1, 1, 1, 1, 1, 10, 10, 10, 10, 15, 20, 20, 30, 40, 55, 55,
            55, 70, 85};
    private static final int[] ROCKS_ORES_XPS = {5, 5, 5, 17, 17, 17, 17, 17, 26, 35, 35, 40, 0, 50, 65, 80, 80, 80, 95,
            125};
    private static final String[] ROCKS_ORES_NAMES = {"rune essence", "clay", "clay", "copper", "copper", "tin", "tin",
            "bluerite", "limestone", "iron", "iron", "silver", "elemental", "coal", "gold", "mithril", "mithril",
            "mithril", "adamant", "runite"};
    private static final int[] PICKAXES = {1275, 1271, 1273, 1269, 1267, 1265};
    private static final int[] PICKAXE_ANIMATIONS = {624, 628, 629, 627, 626, 625};
    ...
    public static boolean prospect(final Player player, final int type, final int x, final int y) {
        if(!isRock(type)){
            return false;
        }else{
            if(player.isBusy()){
            ...
            World.getWorld().submit(new Event(PROSPECTING_DELAY) {
                @Override
                public void execute() {
                    player.getActionSender().sendMessage("This rock contains " + name + ".");
                    stop2();
                }
    ...
    public static boolean mine(final Player player, final int objectID, final int objectX, final int objectY) {
        if(objectID == 450){
            player.getActionSender().sendMessage("That rock contains no ore.");
            return true;
        }
        if(!isRock(objectID)){
            return false;
            ...
            if(player.getSkills().getLevel(14) < getOreLevel(objectID)){
                player.getActionSender().sendMessage("You need a mining level of " + getOreLevel(objectID) + " to mine this rock.");
                return true;
            }
            final int oreType = getOre(objectID);
            if(oreType == -1){
                player.getActionSender().sendMessage("You cannot mine that ore.");
                return true;
            }
            final GameObject g = World.getWorld().getObjectMap().getObjectAt(objectX, objectY, player.getLocation().getZ());
            ...
            ContentEntity.startAnimation(player, PICKAXE_ANIMATIONS[pickaxe]);
            World.getWorld().submit(new Event(MINING_DELAY) {
                ...
                public void execute() {
                    ...
                    if(g != null && g.getType() == EXPIRED_ORE){
                        stop2();
                        player.getActionSender().sendMessage("There is currently no ore available in this rock.");
                        return;
                    }
                    if(ContentEntity.freeSlots(player) == 0){
                        player.getActionSender().sendMessage("Your inventory is too full to hold any more " + getOreName(objectID) + ".");
                        stop2();
                    }else{
                        ...
                        if(ContentEntity.random(((player.getSkills().getLevel(14) / 10) + 11)) >= (11 + (ContentEntity.random(getOreLevel(objectID)) / 10))){
                            ...
                            player.getActionSender().sendMessage("You manage to mine some " + getOreName(objectID) + ".");
                            final int xp = getOreXp(objectID);
                            ContentEntity.addSkillXP(player, xp * EXPMULTIPLIER, 14);
     ...
     * @return The ore id.
     */
    public static int getOre(final int rock) {
        int ct = 0;
        for(final int obj : ROCKS){
            if(obj == rock){
     ...
     */
    public static int getOreLevel(final int rock) {
        int ct = 0;
        for(final int obj : ROCKS){
            if(obj == rock){
     ...
     */
    public static boolean isRock(final int id) {
     ...
     */
    public static String getOreName(final int rock) {
        int ct = 0;
        for(final int obj : ROCKS){
            if(obj == rock){
     ...
     * @return
     */
    public static int getOreXp(final int rock) {
        int ct = 0;
        for(final int obj : ROCKS){
            if(obj == rock){
     ...
     */
    public static int hasPickaxe(final Player client) {
        int ct = 0;
        final int level = client.getSkills().getLevel(14);
        for(final int id : PICKAXES){
     ...
     */
    public static int getPickLevel(final int pickaxe) {
        int ct = 0;
        for(final int obj : PICKAXES){
            if(obj == pickaxe){
