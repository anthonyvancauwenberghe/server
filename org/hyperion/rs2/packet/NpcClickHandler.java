...
public class NpcClickHandler {
    ...
    public static void handle(final Player player, final int type, final int slot) {
    ...
    private static void handleOption3(final Player player, final int slot) {
     ...
     */
    private static void handleOption1(final Player player, final int slot) {
        if(slot <= 0 || slot >= Constants.MAX_NPCS)
            return;
        final NPC npc = (NPC) World.getWorld().getNPCs().get(slot);
        ...
        if(World.getWorld().getContentManager().handlePacket(10, player, npc.getDefinition().getId(), npc.getLocation().getX(), npc.getLocation().getY(), slot)){
            return;
        }
     ...
     * @param player The player.
     */
    private static void handleOption2(final Player player, final int slot) {
        if(slot <= 0 || slot >= Constants.MAX_NPCS)
            return;
        final NPC npc = (NPC) World.getWorld().getNPCs().get(slot);
        if(npc == null || npc.getLocation().distance(player.getLocation()) > 2)
            return;
        // System.out.println("id: "+npc.getDefinition().getId());
        if(World.getWorld().getContentManager().handlePacket(11, player, npc.getDefinition().getId(), npc.getLocation().getX(), npc.getLocation().getY(), slot))
            return;
        switch(npc.getDefinition().getId()){
            case 1846://godwars bridge to zammy
                if(player.godWarsKillCount[1] < 40){
