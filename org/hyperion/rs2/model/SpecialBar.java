...
public class SpecialBar {
    ...
    private static final int[] IDS = {7812, 12335, 7586, 7611, 7561, 8505, 7511, 7486, 7711, 7636, 8505, 12335,
    ...
    public SpecialBar(final Player player) {
        this.player = player;
    }
     ...
     * break bar?
     */
    public void surpassincrement(final int increase) {
        amount += increase;
    }
    ...
    private void sendColouredBar(int barId) {
        final int sendAmount = amount / 10;
        for(int i = 10; i > 0; i--){
    ...
    public void sendSpecAmount() {
        ...
        for(int i = 0; i < IDS.length; i++){
            player.getActionSender().sendString(IDS[i], color + "Special Attack (" + amount + "%)");
        }
