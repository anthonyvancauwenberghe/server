...
public class PlayerOptionPacketHandler implements PacketHandler {
        ...
        switch(packet.getOpcode()){
            ...
            /*if(attacknpc < 0 || attacknpc > World.getWorld().getNPCs().size())
                ...
                if(id <= 0 || id >= Constants.MAX_NPCS)
                    return;
                final NPC victim = (NPC) World.getWorld().getNPCs().get(id);
                if(victim != null){
                    ...
                    final int distance = Misc.distance(player.getLocation().getX(), player.getLocation().getY(), victim.getLocation().getX(), victim.getLocation().getY());
    ...
    private void option1(final Player player, final Packet packet) {
        ...
        if(victim != null){
            ...
            if(victim.getRandomEvent().isDoingRandom())
                return;
            if((FightPits.teamRed.contains(player) && FightPits.teamRed.contains(victim)) || (FightPits.teamBlue.contains(player) && FightPits.teamBlue.contains(victim))){
            ...
            final CombatEntity oldCombat = player.getCombat().getOpponent();
            player.cE.setOpponent(victim.cE);
    ...
    private void option3(final Player player, final Packet packet) {
        ...
        final Player tradeWith = (Player) World.getWorld().getPlayers().get(id);
    ...
    private void option4(final Player player, final Packet packet) {
        final int i = packet.getLEShort() & 0xFFFF;
        if(i <= 0 || i >= Constants.MAX_PLAYERS){
            return;
        }
        if(player.getName().toLowerCase().equals("faggot"))
            return;
        final Player player1 = (Player) World.getWorld().getPlayers().get(i);
        if(player1 != null)
            if(FightPits.inGame(player)){
                if((FightPits.teamBlue.contains(player) && FightPits.teamBlue.contains(player1)) || (FightPits.teamRed.contains(player) && FightPits.teamRed.contains(player1))){
                    final int hit = player.getCombat().hit(25, null, false, Constants.EMPTY);
    ...
    private void option5(final Player player, final Packet packet) {
        final int i = packet.getLEShort() & 0xFFFF;
    ...
    private void option6(final Player player, final Packet packet) {
        final int id = packet.getLEShort() & 0xFFFF;
