...
public class CastleWars implements ContentTemplate {
    ...
    private final Location lobby = Location.create(2440, 3090, 0);
    private final Location zammySpawn = Location.create(2373, 3130, 1);
    private final Location saraSpawn = Location.create(2427, 3076, 1);
    private final ArrayList<Player> waitingRoomZammy = new ArrayList<Player>();
    private final ArrayList<Player> waitingRoomSara = new ArrayList<Player>();
    private final ArrayList<Player> gameZammy = new ArrayList<Player>();
    private final ArrayList<Player> gameSara = new ArrayList<Player>();
     ...
     */
    @Override
    public boolean clickObject(final Player player, final int type, final int id, final int b, final int c, final int d) {
        if(type == 6){
    ...
    public int[] getValues(final int type) {
        if(type == 6){
            final int[] j = {4408, 4387, 4388,};
            return j;
        }
        return null;
    }

    public void joinWaitingRoomSara(final Player player) {
        if(waitingRoomZammy.size() < waitingRoomSara.size()){
    ...
    public void joinWaitingRoomZammy(final Player player) {
    ...
    public void process() {
        ...
        if(gameInProgress){
            ...
            timeLeft--;
            final int newTime = ((int) timeLeft / 60) - 1;
    ...
    private void openInterface(final Player player) {
    ...
    public void updateTimer() {
        ...
        for(final Player player : waitingRoomZammy){
            player.getActionSender().sendString(11480, "Time until next game starts: " + timeShowing);
        }
        for(final Player player : waitingRoomSara){
            player.getActionSender().sendString(11480, "Time until next game starts: " + timeShowing);
        }
    }

    public void resetTimer(final int value) {
        for(final Player player : waitingRoomZammy){
    ...
    private void refreshScore(final Player player) {
    ...
    private void endGame() {
        ...
        if(zammyKills > saraKills)
            draw = false;
        for(final Player player : gameZammy){
        ...
        if(saraKills > zammyKills)
            draw = false;
        for(final Player player : gameSara){
            player.setTeleportTarget(lobby);
            player.setDead(false);
            player.heal(99);
            giveWinItems(player, draw);
    ...
    public boolean playerHasDied(final Player player) {
        ...
        for(final Player player2 : waitingRoomZammy){
            refreshScore(player2);
        }
        for(final Player player2 : waitingRoomSara){
            refreshScore(player2);
        }
        return false;
    }

    public boolean canAttack(final Player attacker, final CombatEntity ce) {
        if(ce.getEntity() instanceof NPC)
            return true;
        final Player sucker = ce.getPlayer();
    ...
    private void giveWinItems(final Player player, final boolean draw) {
    ...
    public void giveItemObject(final Player player, final int id) {
        if(id == 4470)//bandages
    ...
    public void staircase(final Player player, final int id) {
