...
public class ItemDegrading {
    ...
    private static void checkPvpDegrades(final Player p) {
        ...
        final Item body = p.getEquipment().get(Equipment.SLOT_CHEST);
        if(body != null){
            switch(body.getId()){
                case 13887:
                case 13884:
        ...
        final Item legs = p.getEquipment().get(Equipment.SLOT_BOTTOMS);
        if(legs != null){
            switch(legs.getId()){
                case 13893:
                case 13890:
        ...
        final Item helm = p.getEquipment().get(Equipment.SLOT_HELM);
        if(helm != null){
            switch(helm.getId()){
                case 13896:
                case 13876:
    ...
    private static void checkChaoticDegrades(final Player p) {
        final Item weapon = p.getEquipment().get(Equipment.SLOT_WEAPON);
        if(weapon == null)
            return;
        switch(weapon.getId()){
