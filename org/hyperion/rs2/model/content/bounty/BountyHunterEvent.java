...
public class BountyHunterEvent extends Event {
    ...
    public void execute() {
        ...
        if(counter == 0){
            ...
            World.getWorld().getPlayers().stream().filter(p -> p.cE.getOpponent() == null || !p.getCombat().getOpponent().getEntity().equals(p.getBountyHunter().getTarget())).forEach(p -> p.getBountyHunter().clearTarget());
        }
        if(counter % 2 == 0){
