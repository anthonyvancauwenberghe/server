...
import org.hyperion.rs2.model.shops.CurrencyShop;
import org.hyperion.rs2.model.shops.DeviousHonorShop;
import org.hyperion.rs2.model.shops.DonatorShop;
import org.hyperion.rs2.model.shops.DungeoneeringStore;
...
import org.hyperion.rs2.model.shops.RecklessHonorShop;
...
public class ShopManager {
     ...
     */
    public static void open(final Player player, final int id) {
     ...
     * @param amount
     */
    public static void sellItem(final Player player, final int itemId, final int slot, int amount) {
        ...
        final int has_amount = player.getInventory().getCount(itemId);
        if(has_amount == 0)
            return;
        if(amount > has_amount)
            amount = has_amount;
        final Shop shop = Shop.forId(player.getShopId());
     ...
     * @param itemId
     */
    public static void valueSellItem(final Player player, final int itemId) {
        if(player.getShopId() < 0 || itemId == Shop.COINS_ID)
            return;
        final Item item = player.getInventory().getById(itemId);
        if(item == null)
            return;
        final Shop shop = Shop.forId(player.getShopId());
     ...
     * @param itemId
     */
    public static void valueBuyItem(final Player player, final int itemId) {
        if(player.getShopId() < 0 || itemId == 995)
            return;
        final Shop shop = Shop.forId(player.getShopId());
        final Item item = shop.getContainer().getById(itemId);
        if(item == null)
            return;
    ...
    public static void buyItem(final Player player, final int itemId, final int slot, int amount) {
        ...
        final Shop shop = Shop.forId(player.getShopId());
        final Item shopItem = shop.getContainer().getById(itemId);
        if(shopItem == null)
            return;
     ...
     * @return
     */
    public static int getPoints(final int shopId, final int itemId) {
        switch(shopId){
            case 75:
            case 76:
                switch(itemId){
    ...
    public static boolean isValuable(final int id) {
        if(id == 15272)
            return false;
        for(final int shopid : valuableShopsIds){
            if(getPoints(shopid, id) != 50000 || getPoints(shopid, id + 1) != 50000 || getPoints(shopid, id - 1) != 50000)
                return true;
        }
        return false;
    }

    public static int getSpecialPrice(final int itemid) {
        switch(itemid){
            case 2577:
            case 2581:
                return 4500;
    ...
    public static List<Shop> forName(String key) {
        key = key.toLowerCase();
        final List<Shop> list = new LinkedList<Shop>();
     ...
     * @throws IOException
     */
    private static void loadShops(final String name) throws IOException {
        ...
        while((line = in.readLine()) != null){
            try{
                ...
                final int shopId = Integer.parseInt(parts[0]);
                final String shopName = parts[1];
                final String type = parts[2];
                Shop shop = null;
                final Container shopContainer = new Container(Container.Type.ALWAYS_STACK, SIZE);
                ...
                if(type.contains("donator")){
                    ...
                    shop = new SlayerShop(shopId, shopName, shopContainer);
                }else if(type.contains("emblem")){
                    shop = new EmblemShop(shopId, shopName, shopContainer);
                ...
                for(int i = 3; i < parts.length; i++){
                    ...
                    final int amount = Integer.parseInt(sub[1]);
