...
public class GenericWorldLoader implements WorldLoader {
                ...
                if(MergedSaving.existsMain(name)){
                    ...
                    if(code == LoginResult.INVALID_USER_OR_PASS){
                        ...
                        if(MergedSaving.existsArtero(name) && MergedSaving.existsInstant(name)){
                            final int arteroPriority = MergedSaving.getArteroPriority(name);
                            final int instantPriority = MergedSaving.getInstantPriority(name);
                            if(instantPriority > arteroPriority){
                                //The merge is from instant, let's check Artero
                                final String passStr = MergedSaving.getArteroPass(name);
                                ...
                                if(pass.getSalt() != null && !pass.getSalt().equalsIgnoreCase("null")){
                                    //Encrypt pd pass
                                    final String encryptedPdPass = Password.encryptPassword(pd.getPassword(), pass.getSalt());
                    ...
                    final boolean instant = instantPriority > arteroPriority;
                    if(instant){
                        /*
						 * First assume he logs in with InstantPk account
						 */
                        final Password pass = MergedSaving.getInstantPass(pd.getName());
                        if(pass.getSalt() != null && !pass.getSalt().equalsIgnoreCase("null")){
                            //Encrypt pd pass
                            final String encryptedPdPass = Password.encryptPassword(pd.getPassword(), pass.getSalt());
                            //Compare encrypted passwords
                        ...
                        if(code == LoginResult.INVALID_USER_OR_PASS){
                            if(MergedSaving.existsArtero(name)){
                                final String password = MergedSaving.getArteroPass(name);
    ...
    @Override
    public boolean savePlayer(final Player player, final String info) {
        ...
        TextUtils.writeToFile(file, String.format("[%s]%s has logged out info: %s", new Date(System.currentTimeMillis()).toString(), player.getName(), info));
