...
public class Cooking implements ContentTemplate {
    ...
    public boolean isCookItem(final int item) {
        for(final CookingItem c : cookingItems){
            final int id = c.getCookId();
            if(id == item)
                return true;
        }
        return false;
    ...
    public boolean cookItem(final Player client, final int item, final boolean fireCook, final int amount) {
        ...
        World.getWorld().submit(new Event(2500) {
            ...
            public void execute() {
                ...
                final String name = ContentEntity.getItemName(item);
    ...
    public int[] getValues(final int type) {
        if(type == 0){
            final int[] j = {13720, 13719, 13718, 13717,};
            return j;
        }
        if(type == 14){
            final int[] j = new int[cookingItems.size()];
            int i = 0;
            for(final CookingItem cI : cookingItems){
                if(cI.getCookId() > 0){
                    j[i] = cI.getCookId();
                    i++;
                }
    ...
    public boolean clickObject(final Player client, final int type, final int itemId, final int slot, final int objId, final int a) {
        if(type == 0){
            final boolean fire = (Boolean) client.getExtraData().get("fire");
            final int id = (Integer) client.getExtraData().get("cookFishId");
            if(itemId == 13720)
