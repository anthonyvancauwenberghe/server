...
public class SummoningData {
    ...
    private static final Map<Integer, SummonType> summonsNpcId = new HashMap<Integer, SummonType>();

    ;
    private static final Map<Integer, SummonType> summonsPouchId = new HashMap<Integer, SummonType>();

    static {
        for(final SummonType summonType : SummonType.values()){
            summonsNpcId.put(summonType.npcId, summonType);
            summonsPouchId.put(summonType.pouchId, summonType);
        }
    }

    public static int getTimerById(final int npcId) {
        if(summonsNpcId.get(npcId) == null)
            return 0;
        return summonsNpcId.get(npcId).timeLiving;
    }

    public static int getPouchByNpc(final int npcId) {
        if(summonsNpcId.get(npcId) == null)
            return 0;
        return summonsNpcId.get(npcId).pouchId;
    }

    public static int getNpcbyPouchId(final int pouchId) {
        if(summonsPouchId.get(pouchId) == null)
            return 0;
        return summonsPouchId.get(pouchId).npcId;//+1 is u can attack in wild
    }

    public static int getRequirementForNpcId(final int npcId) {
        if(summonsNpcId.get(npcId) == null)
            return 0;
    ...
    public enum SummonType {
        ...
        REV_HELLHOUND(6688, 17985, 70, (int) Time.TEN_MINUTES, MAGIC); //rev knight

        public int npcId, pouchId, level, timeLiving, skillId;

        /* Npc id, Pouch id, Name, Level required, Time living */
        SummonType(final int npcId, final int pouchId, final int level, final int timeLiving, final int skillId) {
            this.npcId = npcId;
