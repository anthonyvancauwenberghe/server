...
public class Food implements ContentTemplate {
     ...
     */
    public final int ANIMATION_EAT_ID = 829, ANIMATION_DRINK_ID = 829;//1652

    public static FoodItem get(final int id) {
        for(final FoodItem i : foods){
            if(i.getId() == id){
                return i;
            }
        }
    ...
    public int[] getValues(final int type) {
        if(type != 1)
            return null;
        final int[] j = new int[foods.size()];
        int i2 = 0;
        for(final FoodItem i : foods){
            j[i2++] = i.getId();
        }
    ...
    public void antiFire(final Player player, final boolean superFire) {
    ...
    public boolean clickObject(final Player player, final int type, final int id, final int slot, final int c, final int d) {
        ...
        if(System.currentTimeMillis() - (player.comboFoodTimer) < 1500 || player.isDead())
            return true;
        for(final int comboFood : COMBO_FOODS){
            if(id == comboFood){
                switch(id){
                    case 3144:
        ...
        if(System.currentTimeMillis() - (!foodItem.isDrink() ? player.foodTimer : player.potionTimer) < 1500 || player.isDead())
            return true;
        if(!ContentEntity.isItemInBag(player, id, slot))
            return true;
        if(!foodItem.isDrink()){
        ...
        if(foodItem.getSkillId() == 5){
            final int standardLevel = ContentEntity.getLevelForXP(player, 5);
            final int levelAtm = ContentEntity.returnSkillLevel(player, 5);
        ...
        if(foodItem.getSkillId() == 21){
            for(int i = 0; i < Skills.SKILL_NAME.length; i++){
                if(i == 3)
                    continue;
                final int standardLevel = ContentEntity.getLevelForXP(player, i);
                final int levelAtm = ContentEntity.returnSkillLevel(player, i);
                if(levelAtm < standardLevel){
    ...
    private void rocktail(final Player player, final int slot) {
        int heal = 23;
        if(player.getEquipment().get(Equipment.SLOT_AMULET) != null && player.getEquipment().get(Equipment.SLOT_AMULET).getId() == WildernessBossEvent.NECKLACE_ID)
            heal *= 1.1;
        final int newHpLevel = player.getSkills().getLevel(Skills.HITPOINTS) + heal;
    ...
    private void eatComboFood(final Player player, final int slot, final int id, final int healAmt, final String foodName) {
    ...
    private void saraBrew(final Player client) {
