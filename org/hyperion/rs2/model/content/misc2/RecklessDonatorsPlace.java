package org.hyperion.rs2.model.content.misc2;

import org.hyperion.rs2.model.Location;
import org.hyperion.rs2.model.NPC;
import org.hyperion.rs2.model.Player;
import org.hyperion.rs2.model.World;
import org.hyperion.rs2.model.content.ContentTemplate;

import java.io.FileNotFoundException;

public class RecklessDonatorsPlace implements ContentTemplate {

    public final static Location LOCATION = Location.create(2313, 9809, 0);

    public static NPC DICER = World.getWorld().getNPCManager().addNPC(2528, 4779, 0, 2999, -1);

    @Override
    public boolean clickObject(final Player player, final int type, final int a, final int b, final int c, final int d) {
        if(type == 10 || type == 11){
            /*switch(a){
            case 3199:
				Bank.open(player, false);
				break;
			case 6167:
				ShopManager.open(player, 74);
				break;
			}*/
        }
        return false;
    }

    @Override
    public void init() throws FileNotFoundException {
        addNPC(3199, 3170, 9756, 0);
        addNPC(6167, 3163, 9757, 0);
    }

    private void addNPC(final int id, final int x, final int y, final int z) {
        World.getWorld().getNPCManager().addNPC(x, y, z, id, -1);
    }

    @Override
    public int[] getValues(final int type) {
        if(type == 10 || type == 11){
            final int[] npcs = {3199, 6167};
            return npcs;
        }
        return null;
    }

}
