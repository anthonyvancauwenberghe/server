...
public class Container {
     ...
     */
    private final Item[] previousItems;
    /**
     * A list of listeners.
     */
    ...
    public static boolean transfer(final Container from, final Container to) {
        for(int i = 0; i < from.capacity(); i++){
            final Item fromItem = from.get(i);
            if(fromItem == null)
                continue;
            if(to.add(fromItem)){
                from.set(i, null);
    ...
    public LinkedList<Integer> getChangedSlots() {
        ...
        for(int i = 0; i < capacity; i++){
            ...
            final Item previousItem = previousItems[i];
            if(item != null){
        ...
        if(item.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)){
            for(int i = 0; i < items.length; i++){
                if(items[i] != null && items[i].getId() == item.getId()){
                    ...
                    final long fuck_all_count = BigInteger.valueOf(item.getCount()).add(BigInteger.valueOf(items[i].getCount())).longValueExact();
    ...
    public boolean add(final Item item, final int slot) {
        final Item i = get(slot);
        if(i == null){
            set(slot, item);
            fireItemsChanged();
            return true;
