...
public class Thieving implements ContentTemplate {
    ...
    public static final int[] pickPocketNpcs = {1, 2, 3, 4, 5, 6, 7, 9, 15, 16, 18, 20, 21, 23, 24, 25, 26, 32, 34, 66,
            67, 68, 159, 160, 161, 168, 169, 170, 187, 296, 297, 298, 299, 617, 646, 1086, 1128, 1129, 1305, 1306, 1307,
            1308, 1309, 1310, 1311, 1312, 1313, 1314, 1528, 1670, 1714, 1715, 1757, 1758, 1880, 1881, 1883, 1884, 1888,
            1889, 1890, 1892, 1893, 1894, 1896, 1897, 1898, 1905, 1926, 1931, 2082, 2234, 2235, 2256, 2363, 2364, 2365,
            2366, 2367, 2675, 2683, 2684, 2699, 2700, 2701, 2702, 2703, 3112, 3222, 3223, 3224, 3225, 3226, 3227, 3228,
            3229, 3230, 3231, 3232, 3233, 3237, 3238, 3239, 3240, 3241, 3407, 3408, 3915, 4307, 4308, 4309, 4310, 4311,
            5589, 5590, 5591, 5592, 5606, 5752, 5753, 5754, 5755, 5756, 5757, 5758, 5759, 5760, 5761, 5762, 5763, 5764,
            5765, 5766, 5767, 5768, 5769, 5825, 5919, 5920, 5923, 5924, 6002,};
    private static final int[] guardIds = {9, 10, 32, 21, 23, 26};
    //private List<PickpocketNpc> pickpockets;
    private final int STUN_TIMER = 6000, STUN_GFX = 245, STEAL_ANIM = 881;
    ...
    public int getStallIndex(final int stall) {
    ...
    public void pickPocketNpc(final Player player, final NPC npc) {
    ...
    public boolean stealFromStall(final Player player, final int objectId, final int x, final int y) {
        ...
        final Stall stall = stalls.get(getStallIndex(objectId));
        ...
        if(face == null)
            face = 10;
        final int item = ContentEntity.random(stall.getItems().length - 1);
        ...
        final NPC guard = getGuard(player);
        if(guard != null){
    ...
    public NPC getGuard(final Player player) {
        boolean caught = true;
    ...
    public int[] getValues(final int type) {
        ...
        if(type == 7){
            final int[] stall_ids = new int[stalls.size()];
            int idx = 0;
