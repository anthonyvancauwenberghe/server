...
public class SummoningMonsters {
    ...
    public static NPCDefinition loadDefinition(final int id) {
        ...
        switch(id){
            ...
            case 6869:
                return NPCDefinition.create(id, 250, 139, new int[]{10, 10, 10, 10, 10, 10, 10, 10, 10,
                        10}, 8305, 8304, new int[]{8303}, 1, "Wolpertinger", 0);
            case 6873:
                return NPCDefinition.create(id, 250, 239, new int[]{10, 10, 10, 10, 10, 10, 10, 10, 10,
                        10}, 0, 0, new int[]{0}, 1, "Pack Yak", 0);
        }
        return null;
    }

    public static void runEvent(final Player p) {
        ...
        final int distance = Misc.distance(p.getLocation().getX(), p.getLocation().getY(), p.cE.summonedNpc.getLocation().getX(), p.cE.summonedNpc.getLocation().getY());
        if(distance > 8){
            final Location newlocation = p.getLocation().getCloseLocation();
    ...
    public static void SummonNewNPC(final Player p, final int npcID, final int itemId) {
        final int req = SummoningData.getRequirementForNpcId(npcID);
        if(p.getSkills().getLevel(23) < req){
            p.getActionSender().sendMessage("You need a Summoning Level of " + req + " to summon this npc.");
            return;
        }
    ...
    public static void SummonNewNPC2(final Player p, final int npcID) {

        final NPC monster = World.getWorld().getNPCManager().addNPC(p.getLocation().getX(), p.getLocation().getY(), p.getLocation().getZ(), npcID, -1);
    ...
    public static void renewFamiliar(final Player player) {
        if(player.SummoningCounter <= 0)
            return;
        if(player.cE.summonedNpc == null)
            return;
        final int npcId = player.cE.summonedNpc.getDefinition().getId();
        if(npcId <= 0)
            return;
        final int pouchId = SummoningData.getPouchByNpc(npcId);
    ...
    public static void bobToInventory(final Player player) {
        if(player.SummoningCounter <= 0)
            return;
        final int size = player.getBoB().size();
        if(size > 0){
            ...
            for(int index = 0; index < amountToTransfer; index++){
                if(player.getInventory().size() == player.getInventory().capacity())
                    break;
                final Item item = player.getBoB().get(index);
    ...
    public static void openSummonTab(final Player player, final NPC npc) {
    ...
    public static void refreshSummonTab(final Player player, final NPC npc) {
    ...
    public static String getTimeForTick(final int tick) {
        if(tick <= 0)
            return "0:00";
        final long ms = tick * 600;
        final int totalMinutes = (int) (ms / 1000);
        final int hourLeft = (totalMinutes) / 60;
        final int minutesLeft = totalMinutes % 60;
        final String hour = (hourLeft > 10) ? ("" + hourLeft) : ("0" + hourLeft);
        final String minute = (minutesLeft > 10) ? ("" + minutesLeft) : ("0" + minutesLeft);
        return hour + ":" + minute;
    }
