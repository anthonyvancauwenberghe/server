...
public class HideNSeek {
    ...
    private final static int CYCLE_TIME = 60000;
    ...
    private Event HIDENSEEK_EVENT = new Event(CYCLE_TIME) {
        @Override
        public void execute() {
    ...
    public void startHideNSeek(Player player) {
        ...
        Events.fireNewEvent("HideNSeek", true, 0, Location.create(3074, 3504, 0));
        System.out.println(CYCLE_TIME);
        World.getWorld().submit(HIDENSEEK_EVENT = new Event(CYCLE_TIME) {
                    @Override
                    public void execute() {
                        if (!CHECKWINNER.isRunning())
                            World.getWorld().submit(CHECKWINNER);
    ...
    public void stopHideNSeek(Player player) {
        ...
        player.setJoinedHns(false);
        yellMessage(player.getSafeDisplayName() + " ended the event early, there is no winner!");
    }

    public void joinHideNSeek(Player player) {
        player.setJoinedHns(true);
        yellMessage(player.getSafeDisplayName() + " joined the HideNSeek.");
    }
