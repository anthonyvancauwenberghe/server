...
public class QuestTab {
    ...
    static {
        for(int i = 0; i < Rank.values().length; i++){
            ...
            ActionsManager.getManager().submit(getClickId(36 + i), new ButtonAction() {
                ...
                public void handle(final Player player, final int id) {
                    final List<Rank> playerRanks = new ArrayList<>();
        ...
        ActionsManager.getManager().submit(getClickId(9), new ButtonAction() {
            @Override
            public void handle(final Player player, final int id) {
                final double kdr = player.getKDR();
    ...
    public QuestTab(final Player player) {
        this.player = player;
    }
    ...
    public void sendPlayerCount() {
        final int players = (int) (World.getWorld().getPlayers().size() * World.PLAYER_MULTI);
        final int id = getId(1);    //Easier to adjust later
    ...
    public void sendUptime() {
        final int id = getId(4);
        player.getActionSender().sendString((Rank.hasAbility(player, Rank.ADMINISTRATOR) && Objects.equals(Events.eventName, "")) ? "@or1@Uptime: @gre@" + Server.getUptime() : (Objects.equals(Events.eventName, "") ? "" : "@or1@Event: @gre@" + (Events.eventName.length() > 20 ? Events.eventName.substring(0, 20) + "." : Events.eventName)), id);
        player.getActionSender().sendTooltip(id, (Objects.equals(Events.eventName, "") ? "" : "Teleport to event"));
    }

    public void sendKills() {
        final int id = getId(7);
